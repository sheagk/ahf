...
'''
PartType 0, 1, 2, 3: Coordinates, Velcocity, ParticleIDs, Masses
'''
...
for i in h5files:
    if i[-4:] == 'hdf5' or i[-4:] == 'HDF5':
        ...
        if 'PartType4' in f.keys():
            ...
            of.write(np.float32(f['/PartType4/StellarFormationTime'].value))
            of.write(np.uint32(TNp[4]*4))
        ...
        of.write(np.float32(f['/PartType0/Metallicity_00'].value))
        if 'PartType4' in f.keys():
            of.write(np.float32(f['/PartType4/Metallicity_00'].value))
        of.write(np.uint32(Tbp*4))
        ...
        of.write(np.uint32(TNp[0]*4))
        of.write(np.float32(f['/PartType0/FractionH2'].value))
        of.write(np.uint32(TNp[0]*4))
