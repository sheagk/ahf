...
inline static void local_getAllFromInfo(const char *fname, char **path,
                                        char **suffix, int *numFiles,
                                        bool *truncateNrowc);

inline static void local_convertStrgToStais(io_file_strg_struct_t *strg,
                                            stai_t data[6]);

inline static void local_convertUnits(io_file_strg_struct_t *s, double xfac,
                                      double vfac, uint64_t pr);

inline static void local_handleStrgWeightIdU(io_file_strg_struct_t *s,
                                             uint64_t ps, uint64_t pr);
...
#endif
    io_logging_msg(log, INT32_C(1), "New ARTnew reader created.");
    xfree(path);
    xfree(suffix);
...
extern void io_artnew_init(io_logging_t log, io_artnew_t f) {
    ...
    io_logging_msg(log, INT32_C(1), "ARTnew reader initialized.");
}
...
/*--- Prototypes of local functions -------------------------------------*/
static void local_getAllFromInfo(const char *fname, char **path, char **suffix,
                                 int *numFiles, bool *truncateNrowc) {
    FILE *f;
    char buffer[2048];
...
inline static void local_convertStrgToStais(io_file_strg_struct_t *strg,
                                            stai_t data[6]) {
    data[0] = stai_new(strg->posx.val, strg->bytes_float, strg->posx.stride);
    data[1] = stai_new(strg->posy.val, strg->bytes_float, strg->posy.stride);
    data[2] = stai_new(strg->posz.val, strg->bytes_float, strg->posz.stride);
    data[3] = stai_new(strg->momx.val, strg->bytes_float, strg->momx.stride);
    data[4] = stai_new(strg->momy.val, strg->bytes_float, strg->momy.stride);
    data[5] = stai_new(strg->momz.val, strg->bytes_float, strg->momz.stride);
}
...
inline static void local_handleStrgWeightIdU(io_file_strg_struct_t *s,
    ...
    if (s->weight.val != NULL) {
        for (uint64_t i = UINT64_C(0); i < pr; i++) {
            ...
            s->weight.val =
                (void *)(((char *)(s->weight.val)) + s->weight.stride);
        }
    }
    if (s->id.val != NULL) {
