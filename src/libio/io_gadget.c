 ...
 */
inline static io_gadget_t local_openopen(io_logging_t log, io_gadget_t f,
                                         io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_gadget_t f,
 ...
 */
inline static io_gadget_t local_opengetswap(io_logging_t log, io_gadget_t f);
 ...
 * \return Nothing.
 */
inline static void local_openversion(io_logging_t log, io_gadget_t f);
 ...
 * \return
 */
inline static int32_t local_write_common(io_logging_t log, io_gadget_t f,
                                         uint64_t pskip, int32_t bytes);

static uint64_t local_get_block_pos(io_logging_t log, io_gadget_t f,
                                    uint64_t *pskip, uint64_t *pread,
                                    io_file_strg_struct_t strg);

static uint64_t local_get_block_vel(io_logging_t log, io_gadget_t f,
                                    uint64_t pskip, uint64_t pread,
                                    io_file_strg_struct_t strg);

static uint64_t local_get_block_id(io_logging_t log, io_gadget_t f,
                                   uint64_t pskip, uint64_t pread,
                                   io_file_strg_struct_t strg);

static uint64_t local_get_block_mass(io_logging_t log, io_gadget_t f,
                                     uint64_t pskip, uint64_t pread,
                                     io_file_strg_struct_t strg);

static uint64_t local_get_block_u(io_logging_t log, io_gadget_t f,
                                  uint64_t pskip, uint64_t pread,
                                  io_file_strg_struct_t strg);

static uint64_t local_get_block_u_search(io_logging_t log, io_gadget_t f,
                                         uint64_t pskip, uint64_t pread,
                                         io_file_strg_struct_t strg);

static uint64_t local_get_block_z(io_logging_t log, io_gadget_t f,
                                  uint64_t pskip, uint64_t pread,
                                  io_file_strg_struct_t strg);

static uint64_t local_get_block_age(io_logging_t log, io_gadget_t f,
                                    uint64_t pskip, uint64_t pread,
                                    io_file_strg_struct_t strg);

static void local_skip_GADGET1_blocks(FILE *, int, int);
...
extern void io_gadget_close(io_logging_t log, io_gadget_t *f) {
    ...
    if (((*f)->mode == IO_FILE_WRITE) && ((*f)->header != NULL)) {
...
extern void io_gadget_init(io_logging_t log, io_gadget_t f) {
    ...
    io_logging_msg(log, INT32_C(5),
                   "Starting to initialize file object from %s", f->fname);
    f->header = io_gadget_header_get(log, f);
    io_logging_msg(log, INT32_C(5),
                   "Done with initializing file object from %s", f->fname);
...
#endif
    tmp = io_gadget_scale_particles(
        log, f->maxpos, f->minpos, &(f->header->boxsize), f->header->expansion,
        f->posscale, f->mmass, particles_read, strg);
...
#define SKIP                                                                   \
    { io_util_readuint32(f->file, &blocksize, f->swapped); }
#define SKIP2                                                                  \
    { io_util_readuint32(f->file, &blocksize2, f->swapped); }
#define CHECK_BLOCK                                                            \
    {                                                                          \
    ...
    {                                                                          \
        if (f->ver == 2) {                                                     \
            ...
            if (strncmp(name, str, 4) != 0) {                                  \
                io_logging_fatal(log, "Wrong block! Expected: %s  Found: %s",  \
                                 name, str);                                   \
            } else {                                                           \
                io_logging_msg(                                                \
                    log, INT32_C(1),                                           \
                    "Arrived at block %s, size of it will be %" PRIu32, str,   \
                    nextblocksize);                                            \
            }                                                                  \
            SKIP2;                                                             \
            CHECK_BLOCK;                                                       \
...
extern uint64_t io_gadget_readpart_raw(io_logging_t log, io_gadget_t f,
...
#ifdef FOPENCLOSE
    // fprintf(stderr,"FOPENCLOSE: io_gadget_readpart_raw() opening %s ...
    // ",f->fname);
    f->file = fopen(f->fname, IO_FILE_MODE_READ);
    if (f->file == NULL) {
        io_logging_fatal(
            log, "io_gadget_readpart_raw(): could not open file %s for reading",
            f->fname);
        return UINT64_C(0);
    }
#endif
    ...
    if (funcrtn != pread) {
        io_logging_fatal(log, "Expected to read %" PRIu64
    ...
    if (funcrtn != pread) {
        io_logging_fatal(log, "Expected to obtain %" PRIu64
...
#ifdef METALHACK
    if (f->header->np[4] > 0) { // only makes sense to read stellar ages and
                                // metalicities, if there are actually any stars
        // fprintf(stderr,"trying to read %d (np[4]=%d)stellar ages
        // now...\n",pread,f->header->np[4]);
...
#ifdef FOPENCLOSE
    fclose(f->file);
    f->file = NULL; // we are not going to read any more from the file and hence
                    // can indicate to close it by setting f->file=NULL
...
extern bool io_gadget_get(io_logging_t log, io_gadget_t f, io_file_get_t what,
    ...
    case IO_FILE_GET_NOVPART:
        if (f->no_part > UINT64_C(0)) {
            if (isgreater(f->sumweight, 0.0) && isgreater(f->mmass, 0.0))
...
extern void io_gadget_log(io_logging_t log, io_gadget_t f) {
    io_logging_msg(log, INT32_C(5), "Fileobject information:");
    io_logging_msg(log, INT32_C(5), "  Filetype:             %s",
                   io_file_typestr(f->ftype));
    io_logging_msg(log, INT32_C(5), "  Filename:             %s", f->fname);
    io_logging_msg(log, INT32_C(5), "  Mode:                 %" PRIi8, f->mode);
    io_logging_msg(log, INT32_C(5), "  Swapping:             %" PRIi8,
                   f->swapped);
    io_logging_msg(log, INT32_C(5), "  File version:         %" PRIi8, f->ver);
    io_logging_msg(log, INT32_C(5), "  Header size:          %" PRIi8, f->ver);
    io_logging_msg(log, INT32_C(5), "  No. particles:        %" PRIu64,
                   f->no_part);
    io_logging_msg(log, INT32_C(5), "  No. particles w/mass: %" PRIu64,
                   f->no_part_with_mass);
    io_logging_msg(log, INT32_C(5), "  Multimass:            %" PRIi8,
                   f->multimass);
    {
        int i;
        for (i = 0; i < 6; i++)
            io_logging_msg(log, INT32_C(5), "      Part type %d:      %" PRIi8,
                           i, ((int8_t)(f->multimass) & (1 << i)) >> i);
    }
    io_logging_msg(log, INT32_C(5), "  MMass (Halo parts):   %g", f->mmass);
    io_logging_msg(log, INT32_C(5), "  Minimal Weight:       %g", f->minweight);
    io_logging_msg(log, INT32_C(5), "  Maximal Weight:       %g", f->maxweight);
    io_logging_msg(log, INT32_C(5), "  Sum of all weights:   %g", f->sumweight);
    io_logging_msg(log, INT32_C(5), "  No. of species:       %" PRIi32,
                   f->no_species);
    io_logging_msg(log, INT32_C(5), "  Position scale:       %g", f->posscale);
    io_logging_msg(log, INT32_C(5), "  Weight scale:         %g",
                   f->weightscale);
...
extern void io_gadget_resetscale(io_logging_t log, io_gadget_t f,
    ...
    io_logging_msg(log, INT32_C(8), "Old posscale: %g   New posscale: %g",
                   f->posscale, posscale);
    io_logging_msg(log, INT32_C(8), "Old weightscale: %g   New weightscale: %g",
...
extern uint64_t io_gadget_scale_particles(io_logging_t log, double maxpos[],
    ...
    io_logging_msg(log, INT32_C(4), "Extreme positions: xmin = %g  xmax = %g",
                   minpos[0], maxpos[0]);
    io_logging_msg(log, INT32_C(4), "                   ymin = %g  ymax = %g",
                   minpos[1], maxpos[1]);
    io_logging_msg(log, INT32_C(4), "                   zmin = %g  zmax = %g",
                   minpos[2], maxpos[2]);
    shift[0] = (isless(minpos[0], 0.0) ? -minpos[0] : 0.0);
    shift[1] = (isless(minpos[1], 0.0) ? -minpos[1] : 0.0);
    shift[2] = (isless(minpos[2], 0.0) ? -minpos[2] : 0.0);
    io_logging_msg(log, INT32_C(4), "Applying shift: (%g, %g, %g)", shift[0],
                   shift[1], shift[2]);
    ...
    io_logging_msg(log, INT32_C(3), "             weights:    %g",
                   scale_weight);
...
#define SCALE_CALL(type)                                                       \
    {                                                                          \
        ...
        *((type *)strg.momz.val) *= (type)(scale_mom);                         \
        ...
        if (strg.u.val != NULL) {                                              \
            if (*((type *)strg.u.val) >= 0.)                                   \
                *((type *)strg.u.val) *= (type)(scale_u);                      \
        }                                                                      \
...
extern void io_gadget_scale_global(io_logging_t log, MPI_Comm comm,
    ...
    MPI_Allreduce((void *)maxpos, (void *)buffer, 3, MPI_DOUBLE, MPI_MAX, comm);
    io_logging_msg(log, INT32_C(5),
                   "local : maxpos[0] = %g \t"
                   "maxpos[1] = %g \t"
                   "maxpos[2] = %g",
    ...
    MPI_Allreduce((void *)minpos, (void *)buffer, 3, MPI_DOUBLE, MPI_MIN, comm);
    io_logging_msg(log, INT32_C(5),
                   "local : minpos[0] = %g \t"
                   "minpos[1] = %g \t"
                   "minpos[2] = %g",
    ...
    MPI_Allreduce((void *)mmass, (void *)buffer, 1, MPI_DOUBLE, MPI_MIN, comm);
...
inline static void local_openswapped(io_logging_t log, io_gadget_t f,
    ...
    if (f->mode == IO_FILE_READ) {
        ...
        case IO_FILE_ISNOT_SWAPPED:
            io_logging_msg(log, INT32_C(3), "Assuming unswapped file");
            break;
        case IO_FILE_IS_SWAPPED:
            io_logging_msg(log, INT32_C(3), "Assuming swapped file");
            break;
        case IO_FILE_UNKOWN_SWAPPING:
        default:
            io_logging_msg(log, INT32_C(3), "Will try to find out swap status");
        }
    }
...
inline static io_gadget_t local_opengetswap(io_logging_t log, io_gadget_t f) {
    ...
    io_logging_msg(log, INT32_C(0),
                   "Boundary (file): %" PRIu32 " Boundary (swapped): %" PRIu32,
                   tmp, bbound1);
    ...
    if (bbound1 > tmp) {
        ...
        io_logging_msg(log, INT32_C(0), "Trying nonswapped...");
    } else {
        f->swapped = IO_FILE_IS_SWAPPED;
        io_logging_msg(log, INT32_C(0), "Trying swapped...");
    }

    io_logging_msg(log, INT32_C(0), "Will skip %" PRIu32 " bytes...", bbound1);
    ...
    if (io_util_readuint32(f->file, &bbound2, f->swapped) == 0) {
        io_logging_fatal(log, "Could not read the second block delimiter. "
                              "Corrupt file?");
        return NULL;
    }

    io_logging_msg(log, INT32_C(0), "Second boundary (file): %" PRIu32,
                   bbound2);

    if (bbound1 == bbound2) {
        if (f->swapped == IO_FILE_IS_SWAPPED) {
            io_logging_msg(log, INT32_C(2), "Identified a byte swapped file.");
        } else {
...
static uint64_t local_get_block_pos(io_logging_t log, io_gadget_t f,
    ...
    io_logging_msg(log, INT32_C(1),
                   ...
                   "are stored.",
                   f->no_part, bytes_file, (float)(blocksize / 1024. / 1024.));
    ...
    if (f->no_part - *pskip < *pread) {
        *pread = f->no_part - *pskip;
        io_logging_msg(log, INT32_C(3),
                       ...
                       "skipping. Will only read %" PRIu64 "particles.",
                       *pread);
    }
...
static uint64_t local_get_block_vel(io_logging_t log, io_gadget_t f,
    ...
    for (i = 0; i < pread; i++) {
        ...
        strg.momx.val = (void *)(((char *)strg.momx.val) + strg.momx.stride);
        strg.momy.val = (void *)(((char *)strg.momy.val) + strg.momx.stride);
        strg.momz.val = (void *)(((char *)strg.momz.val) + strg.momx.stride);
...
#define BOOK_KEEPING                                                           \
    ...
    if (f->multimass != 0) {
        ...
        io_logging_msg(log, INT32_C(1), "blocksize = %i", (int)blocksize);
#ifdef AHOBBS_GADGET_FILE_FIX
        bytes_file = sizeof(float);
#endif
        CHECK_FLOATBYTES(bytes_file, strg.bytes_float);
    ...
    for (i = 0; i < f->no_part; i++) {
        ...
        if ((long)j >= f->header->np[curprtt]) {
            ...
            io_logging_msg(log, INT32_C(0),
                           ...
                           "i=%" PRIu64 " j=%" PRIu64 " curprtt=%" PRIi32
                           " np[curprtt]=%" PRIu64,
...
#define GET_BLOCK                                                              \
    {                                                                          \
        ...
        io_util_readuint32(f->file, &nextblocksize, f->swapped);               \
        io_logging_msg(log, INT32_C(1),                                        \
                       "Arrived at block %s, size of it will be %" PRIi32,     \
                       str, nextblocksize);                                    \
        SKIP2;                                                                 \
        CHECK_BLOCK;                                                           \
    }
...
static uint64_t local_get_block_u_search(io_logging_t log, io_gadget_t f,
    ...
    if (f->ver != 2) {
        local_skip_GADGET1_blocks(f->file, f->swapped,
                                  4); // we skip the first 4 blocks
    }
...
static uint64_t local_get_block_z(io_logging_t log, io_gadget_t f,
    ...
    if (f->ver != 2) {
        local_skip_GADGET1_blocks(f->file, f->swapped,
                                  11); // we skip the first 11 blocks
    }
    ...
    /* we have a Gadget 2 file and hence use the HEAD to find Z */
        ...
        tries = 0;
        nextblocksize = 0;
        while ((strncmp(str, "Z   ", 4) != 0 && strncmp(str, "ZTOT", 4) != 0) &&
               (tries < 100)) {
#ifdef GADGET_MAGNETICUM
            fprintf(stderr, "skipping block %s of size %" PRIu32 "\n", str,
                    nextblocksize);
    ...
    starsoffset = f->header->np[0] + f->header->np[1] + f->header->np[2] +
                  f->header->np[3];
    if (pskip < f->header->np[0])
        ...
        metalzskip = f->header->np[0] + pskip -
                     starsoffset; // Skip all gas and a few stars
    else
        metalzskip =
            f->header->np[0] + f->header->np[4]; // Skip all gas and all stars
    ...
    fseek(f->file,
          partsize *
              (f->header->np[0] + f->header->np[4] - metalzread - metalzskip),
          SEEK_CUR);

    io_logging_msg(log, INT32_C(1), "A total of %" PRIu64 " metals were read.",
                   metalzread);
...
static uint64_t local_get_block_age(io_logging_t log, io_gadget_t f,
    ...
    /* (assuming the orderting
     * POS,VEL,ID,MASS,UGAS,RHO,NE,NH,HSML,SFR,AGE,METAL) */
    if (f->ver != 2) {
        local_skip_GADGET1_blocks(f->file, f->swapped,
                                  10); // we skip the first 10 blocks
    }
    ...
    io_logging_msg(log, INT32_C(1), "A total of %" PRIu64 " ages were read.",
                   agesread);

    /* Go to the end of the block */
    fseek(f->file, partsize * (f->header->np[4] - agesread - agesskip),
          SEEK_CUR);
...
void local_skip_GADGET1_blocks(FILE *fpgadget, int swapped, int nblocks) {
    ...
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np0=%d\n",idummy);
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np1=%d\n",idummy);
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np2=%d\n",idummy);
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np3=%d\n",idummy);
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np4=%d\n",idummy);
    io_util_readint32(fpgadget, &idummy,
                      swapped); // fprintf(stderr,"np5=%d\n",idummy);
    ...
    io_util_readdouble(fpgadget, &ddummy, swapped);
    io_util_readdouble(fpgadget, &ddummy, swapped);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"expansion=%g\n",ddummy);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"redshift=%g\n",ddummy);
    ...
    io_util_readint32(fpgadget, &idummy, swapped);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"boxsize=%g\n",ddummy);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"omega=%g\n",ddummy);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"omegalambda=%g\n",ddummy);
    io_util_readdouble(fpgadget, &ddummy,
                       swapped); // fprintf(stderr,"hubble=%g\n",ddummy);
    io_util_readint32(fpgadget, &idummy, swapped);
    io_util_readint32(fpgadget, &idummy, swapped);
    io_util_readuint32(fpgadget, &uidummy, swapped);
    io_util_readuint32(fpgadget, &uidummy, swapped);
    ...
    // fprintf(stderr,"skipped HEADER of size %u vs. %u
    ...
    for (iblock = 0; iblock < nskipblocks; iblock++) {
        ...
        // fprintf(stderr,"skipped block #%d (out of %d in total) of size %u vs.
        // %d MB\n",iblock,nblocks,blocksize1/1024/1024,blocksize2/1024/1024);

        /* the file appears to be corrupted */
        if (blocksize1 != blocksize2) {
            fprintf(stderr, "We are already trying to help you with your "
                            "ancient GADGET1 file, but enough is enough!\n");
            exit(-1);
        }
    }
}
