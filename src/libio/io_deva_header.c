...
#define io_util_readintd(f, d, b)                                              \
    {                                                                          \
        if (deva_intsize == sizeof(int64_t)) {                                 \
            int64_t dummyint0;                                                 \
            io_util_readfortran(f, &dummyint0, deva_intsize, 1, 0, b);         \
            (d) = dummyint0;                                                   \
        } else if (deva_intsize == sizeof(int32_t)) {                          \
            int32_t dummyint0;                                                 \
            io_util_readfortran(f, &dummyint0, deva_intsize, 1, 0, b);         \
            (d) = dummyint0;                                                   \
        }                                                                      \
...
#define io_util_readreald(f, d, b)                                             \
    {                                                                          \
        if (deva_realsize == sizeof(double)) {                                 \
            double dummyreal0;                                                 \
            io_util_readfortran(f, &dummyreal0, deva_realsize, 1, 0, b);       \
            (d) = dummyreal0;                                                  \
        } else if (deva_realsize == sizeof(float)) {                           \
            float dummyreal0;                                                  \
            io_util_readfortran(f, &dummyreal0, deva_realsize, 1, 0, b);       \
            (d) = dummyreal0;                                                  \
        }                                                                      \
...
\**********************************************************************/
extern io_deva_header_t io_deva_header_get_native(io_logging_t log,
                                                  io_deva_t f) {
...
extern io_deva_header_t io_deva_header_get(io_logging_t log, io_deva_t f) {
    io_deva_header_t dummy;
    long skipsize;
    int iloop;
...
extern void io_deva_header_del(io_logging_t log, io_deva_header_t *header) {
...
extern void io_deva_header_write(io_logging_t log, io_deva_header_t header,
                                 io_deva_t f) {
...
extern void io_deva_header_log(io_logging_t log, io_deva_header_t header) {
    io_logging_msg(log, INT32_C(5), "  itime:                         %" PRIi32,
                   header->itime);
    io_logging_msg(log, INT32_C(5), "  irun:                          %" PRIi32,
                   header->irun);
    io_logging_msg(log, INT32_C(5), "  nobj:                          %" PRIi32,
                   header->nobj);
    io_logging_msg(log, INT32_C(5), "  ndark:                         %" PRIi32,
                   header->ndark);
    io_logging_msg(log, INT32_C(5), "  ngas:                          %" PRIi32,
                   header->ngas);
    io_logging_msg(log, INT32_C(5), "  nstar:                         %" PRIi32,
                   header->nstar);
    io_logging_msg(log, INT32_C(5), "  ztime:                         %e",
                   1. / header->atime - 1.);
    io_logging_msg(log, INT32_C(5), "  atime:                         %e",
                   header->atime);
    io_logging_msg(log, INT32_C(5), "  b100:                          %e",
                   header->box100);
    io_logging_msg(log, INT32_C(5), "  omega0:                        %e",
                   header->omega0);
    io_logging_msg(log, INT32_C(5), "  xlambda0:                      %e",
                   header->xlambda0);
    io_logging_msg(log, INT32_C(5), "  omegab0:                       %e",
                   header->omegab0);
    io_logging_msg(log, INT32_C(5), "  h100:                          %e",
                   header->h100);
    io_logging_msg(log, INT32_C(5), "  sigma80:                       %e",
                   header->sigma80);
    io_logging_msg(log, INT32_C(5), "  h0t0:                          %e",
                   header->h0t0);
    io_logging_msg(log, INT32_C(5), "  rmtot:                         %e",
                   header->rmtot);
    io_logging_msg(log, INT32_C(5), "  rmdark:                        %e",
                   header->rmdark);
    io_logging_msg(log, INT32_C(5), "  rmgas:                         %e",
                   header->rmgas);
...
extern int io_util_readfortran(FILE *const f, void *const buf0, size_t const ts,
    ...
    size_t i, read = 0, readt = 0, remaining = len * ts, toRead, toSkip,
              remainingSkip = skip * ts; // counter
    unsigned char *buf = (unsigned char *)buf0;

    do {
        if (fread(&s1, sizeof(int32_t), 1, f) != 1)
            return readt;
        //		printf("record size: %d ",s1);
        s1p = ABS(s1);
        if (swap)
            io_util_sexchange(&s1, sizeof(int32_t));
        toRead = s1p;
        ...
        //		printf("will skip %ld until the end.
        //",(s1p-(toSkip+toRead)));
        fseek(f, (s1p - (toSkip + toRead)),
              SEEK_CUR); // Skip to the end of the current slice
        if (fread(&s2, sizeof(int32_t), 1, f) != 1)
            return readt;
        s2p = ABS(s2);
        //		printf("record size: %d ",s2);
        if (swap)
            io_util_sexchange(&s2, sizeof(int32_t));
        if (s1p != s2p) {
