...
\***********************************************************************/
#ifdef WITH_MPI
inline static void local_recalcreadskip(io_file_t f, uint32_t reader,
...
extern io_file_t io_file_open(io_logging_t log, char *fname,
    ...
    if ((reader > 0) && (mode == IO_FILE_READ) && (rank >= reader)) {
        ...
        io_logging_msg(log, INT32_C(2),
                       "Opening %s, a %s file on %" PRIu32 " processes.", fname,
                       io_file_typestr(type), ((reader == 0) ? 1 : reader));
    }

    switch (type) {
    case IO_FILE_AMIGA:
        dummy = (io_file_t)io_amiga_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_ARES:
        dummy = (io_file_t)io_ares_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_ASCII:
        dummy = (io_file_t)io_ascii_open(log, fname, mode, reader);
        break;
    case IO_FILE_CUBEP3M:
        dummy = (io_file_t)io_cubep3m_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_MCUBEP3M:
        dummy = (io_file_t)io_mcubep3m_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_ART:
        dummy = (io_file_t)io_art_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_ARTNEW:
        dummy = (io_file_t)io_artnew_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_GADGET:
        dummy = (io_file_t)io_gadget_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_MGADGET:
        dummy = (io_file_t)io_mgadget_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_DEVA:
        dummy = (io_file_t)io_deva_open(log, fname, swapped, mode, reader, 0);
        break;
    case IO_FILE_DEVANATIVE:
        dummy = (io_file_t)io_deva_open(log, fname, swapped, mode, reader, 1);
        break;
    case IO_FILE_TIPSY:
        dummy = (io_file_t)io_tipsy_open(log, fname, swapped, mode, reader);
        break;
    case IO_FILE_HDF5:
        dummy = (io_file_t)io_hdf5_open(log, fname, swapped, mode, reader);
        break;
    default:
        io_logging_fatal(log, "File format %s not supported for %s!",
...
extern void io_file_close(io_logging_t log, io_file_t *f) {
    ...
    io_logging_msg(log, INT32_C(2), "Closing a %s file.",
...
extern uint64_t io_file_readpart(io_logging_t log, io_file_t f, uint64_t pskip,
...
#ifdef WITH_MPI
    local_recalcreadskip(f, f->size_mycomm, pskip, pread, &pskip_parallel,
                         &pread_parallel);
#else
    ...
    switch (f->ftype) {
    case IO_FILE_AMIGA:
        tmp = io_amiga_readpart(log, (io_amiga_t)f, pskip_parallel,
                                pread_parallel, strg);
        break;
    case IO_FILE_ARES:
        tmp = io_ares_readpart(log, (io_ares_t)f, pskip_parallel,
                               pread_parallel, strg);
        break;
    case IO_FILE_ASCII:
        tmp = io_ascii_readpart(log, (io_ascii_t)f, pskip_parallel,
                                pread_parallel, strg);
        break;
    case IO_FILE_CUBEP3M:
        tmp = io_cubep3m_readpart(log, (io_cubep3m_t)f, pskip_parallel,
                                  pread_parallel, strg);
        break;
    case IO_FILE_MCUBEP3M:
        tmp = io_mcubep3m_readpart(log, (io_mcubep3m_t)f, pskip_parallel,
                                   pread_parallel, strg);
        break;
    case IO_FILE_ART:
        tmp = io_art_readpart(log, (io_art_t)f, pskip_parallel, pread_parallel,
                              strg);
        break;
    case IO_FILE_ARTNEW:
        tmp = io_artnew_readpart(log, (io_artnew_t)f, pskip_parallel,
                                 pread_parallel, strg);
        break;
    case IO_FILE_GADGET:
        tmp = io_gadget_readpart(log, (io_gadget_t)f, pskip_parallel,
                                 pread_parallel, strg);
        break;
    case IO_FILE_MGADGET:
        tmp = io_mgadget_readpart(log, (io_mgadget_t)f, pskip_parallel,
                                  pread_parallel, strg);
        break;
    case IO_FILE_DEVA:
        tmp = io_deva_readpart(log, (io_deva_t)f, pskip_parallel,
                               pread_parallel, strg, 0);
        break;
    case IO_FILE_DEVANATIVE:
        tmp = io_deva_readpart(log, (io_deva_t)f, pskip_parallel,
                               pread_parallel, strg, 1);
        break;
    case IO_FILE_TIPSY:
        tmp = io_tipsy_readpart(log, (io_tipsy_t)f, pskip_parallel,
                                pread_parallel, strg);
        break;
    case IO_FILE_HDF5:
...
extern uint64_t io_file_writepart(io_logging_t log, io_file_t f, uint64_t pskip,
    ...
    switch (f->ftype) {
    case IO_FILE_AMIGA:
        tmp = io_amiga_writepart(log, (io_amiga_t)f, pskip, pread, strg);
        break;
    case IO_FILE_ARES:
        tmp = io_ares_writepart(log, (io_ares_t)f, pskip, pread, strg);
        break;
    default:
        io_logging_fatal(log, "File format %s not supported for %s!\n",
                         io_file_typestr(f->ftype), __func__);
...
extern uint64_t io_file_writepart_ord(io_logging_t log, io_file_t f,
    ...
    switch (f->ftype) {
    case IO_FILE_AMIGA:
        tmp = io_amiga_writepart_ord(log, (io_amiga_t)f, pskip, pread, nxt_part,
                                     strg);
        break;
    case IO_FILE_ARES:
        tmp = io_ares_writepart_ord(log, (io_ares_t)f, pskip, pread, nxt_part,
                                    strg);
        break;
    default:
        io_logging_fatal(log, "File format %s not supported for %s!",
