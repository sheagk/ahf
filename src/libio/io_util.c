...
extern int io_util_readbytes(FILE *f, void *n, size_t len,
                             io_file_swap_t swap) {
...
extern char *io_util_split_pathfname(const char *str, char **path,
                                     char **stem) {
    int64_t len, i;
...
extern int32_t io_util_findfiles(const char *path, const char *stem,
    ...
    if ((path == NULL) || (stem == NULL) || (format == NULL) ||
        (suffix == NULL) || (fnames == NULL))
        return INT32_C(0);
...
extern bool *io_util_getminearr(int32_t numfiles) {
    int32_t i = 0;
    bool *dummy = NULL;
    int rank = 0;
    int size = 1;
