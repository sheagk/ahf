...
#define AMIGA_HEADER_HEADERSIZE                                                \
    (AMIGA_HEADER_HEADERSTRING * sizeof(char) + 2 * sizeof(long) +             \
     4 * sizeof(int32_t) + 41 * sizeof(double))

/** Defines the number of unused bytes in the header */
#define AMIGA_HEADER_FILLHEADER (AMIGA_HEADER_SIZE - AMIGA_HEADER_HEADERSIZE)
...
struct io_amiga_header_struct {
    ...
    double param_dummy6;
    double param_dummy7;
    double param_dummy8; // empty space that once was used (downwards
                         // compatibility!)
