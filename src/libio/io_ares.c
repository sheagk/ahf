 ...
 */
inline static io_ares_t local_openopen(io_logging_t log, io_ares_t f,
                                       io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_ares_t f,
 ...
 */
inline static bool local_write_common(io_logging_t log, io_ares_t f,
                                      void *weight, void *u, uint64_t pskip,
                                      int32_t bytes_float, int32_t bytes_int);
...
#define show(bf, bs, what)                                                     \
    ...
    if (strg.id.val == NULL) {
        io_logging_warn(log, INT32_C(3), "   id     (read, ignored)");
    } else {
...
#undef show
    ...
    fseek(f->file, (long)(pskip * partsize) + (long)ARES_HEADER_SIZE, SEEK_SET);
...
extern uint64_t io_ares_writepart(io_logging_t log, io_ares_t f, uint64_t pskip,
    ...
    if (!local_write_common(log, f, strg.weight.val, strg.u.val, pskip,
                            strg.bytes_float, strg.bytes_int))
        return UINT64_C(0);
