...
/**
 * @file libutil/parse_ini.c
 * @ingroup libutilFilesIni
 * @brief  This file provides the implementation of the ini parser.
 */
 ...
 *          the section name.
 */
static char *local_parse_secname(const char *line);
 ...
 */
static void local_parse_key(const char *line, char **key_name, char **value);
 ...
 *          could not be found.
 */
static int local_find_section(parse_ini_t ini, const char *section_name);
 ...
 */
static int local_find_key(local_section_t sec, const char *key_name);
...
extern parse_ini_t parse_ini_open(const char *fname) {
    ...
    while (xgetline(&line, &n, dummy->f) > 0) {
        ...
        case '[':
            ...
            /* Get memory for it  */
            dummy->sections =
                xrealloc(dummy->sections,
                         dummy->num_sections * sizeof(local_section_struct_t));
        ...
        default:
            ...
            if (sec == NULL) {
                ...
                sec->num_keys++;
                /* Get memory for it */
                sec->keys = xrealloc(sec->keys, sec->num_keys *
...
static int local_find_section(parse_ini_t ini, const char *section_name) {
    ...
    for (i = 0; i < ini->num_sections; i++) {
        ...
        len = strlen(sec->name);
        /* Check if this is it */
        if ((len == search_len) && (strncmp(section_name, sec->name, len) == 0))
            return i;
    }
...
static int local_find_key(local_section_t sec, const char *key_name) {
    ...
    for (i = 0; i < sec->num_keys; i++) {
        ...
        /* Check if this is it */
        if ((len == search_len) && (strncmp(key_name, key->key_name, len) == 0))
            return i;
    }
