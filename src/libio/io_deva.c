...
#define PSTAR                                                                  \
    (-4.0) /* identifier for star particles; whatever negative value */
#define PDMbndry                                                               \
    (-5.0) /* identifier for star particles; whatever negative value */
 ...
 */
inline static io_deva_t local_openopen(io_logging_t log, io_deva_t f,
                                       io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_deva_t f,
 ...
 */
inline static io_deva_t local_opengetswap(io_logging_t log, io_deva_t f);
 ...
 * \return
 */
inline static int32_t local_write_common(io_logging_t log, io_deva_t f,
                                         uint64_t pskip, int32_t bytes);

extern int io_util_readfortran(FILE *const f, void *const buf0, size_t const ts,
                               size_t const len, size_t const skip,
                               int const swap);

/**********************************************************************\
 *    Implementation of global functions                              *
\**********************************************************************/
extern io_deva_t io_deva_open(io_logging_t log, char *fname,
                              io_file_swap_t swapped, io_file_mode_t mode,
                              uint32_t reader, int native) {
    io_deva_t f;
    ...
    if (native)
        f->ftype = IO_FILE_DEVANATIVE;
    else
        f->ftype = IO_FILE_DEVA;
    ...
    f->ver = 1; // 1 = refers to raw simulation converted to single precision
                // though...
...
extern void io_deva_close(io_logging_t log, io_deva_t *f) {
    /* Catch NULLs */
    if (f == NULL || *f == NULL)
        return;
...
extern void io_deva_init(io_logging_t log, io_deva_t f, int native) {
    if (f == NULL)
        return;
    ...
    if (native)
        f->header = io_deva_header_get_native(log, f);
    else
        f->header = io_deva_header_get(log, f);
    io_logging_msg(log, INT32_C(5),
...
extern uint64_t io_deva_readpart(io_logging_t log, io_deva_t f, uint64_t pskip,
    ...
    if (native)
        particles_read =
            io_deva_readpart_raw_native(log, f, pskip, pread, strg);
    else
    ...
    /* And do the scaling */
#ifdef WITH_MPI
    io_deva_scale_global(log, f->mycomm, f->maxpos, f->minpos, &(f->mmass),
                         &(f->minweight), &(f->maxweight), &(f->sumweight));
#endif

    tmp = io_deva_scale_particles(
        log, f->maxpos, f->minpos, &(f->header->box100), f->header->atime,
        f->header->h0t0, f->posscale, f->mmass, particles_read, strg);
...
#define SKIP                                                                   \
    { io_util_readint32(f->file, &blocksize, f->swapped); }
#define SKIP2                                                                  \
    { io_util_readint32(f->file, &blocksize2, f->swapped); }
#define CHECK_BLOCK                                                            \
    {                                                                          \
...
#define buftodummy(dummy, buf)                                                 \
    {                                                                          \
        if (deva_realsize == sizeof(float))                                    \
            dummy = (double)*((float *)buf);                                   \
        else if (deva_realsize == sizeof(double))                              \
            dummy = (double)*((double *)buf);                                  \
        buf += deva_realsize;                                                  \
    }

extern uint64_t io_deva_readpart_raw_native(io_logging_t log, io_deva_t f,
    ...
    unsigned char *const buf0 =
        malloc(3 * pread * MAX(deva_intsize, deva_realsize));
    ...
    //================================================================================
    // NOTE: we are scaling the thermal energies to (km/sec)^2 right here, right
    // now!
    //=================================================================================
    v_fac = f->header->atime * 100. * f->header->box100 / (f->header->h0t0);
    e_fac = (f->header->atime * f->header->atime) *
            (100. * f->header->box100 / (f->header->h0t0) * 100. *
             f->header->box100 / (f->header->h0t0));
    ...
    nread = io_util_readfortran(f->file, buf0, deva_realsize, pread, pskip,
                                f->swapped);
    buf = buf0;
    f->sumweight = 0.;
    assert(nread == pread);
    for (i = 0; i < pread; i++) {
        ...
        *((float *)strg.weight.val) = (float)fweight;
        ...
        /* STEP 4: Keep track of total mass (in internal units) and various
    ...
    /* Go to the first particle we want to read */
    nread = io_util_readfortran(f->file, buf0, deva_realsize, 3 * pread, pskip,
                                f->swapped);
    buf = buf0;
    ...
    \*******************************************************************/
    /* Go to the first particle we want to read */
    nread = io_util_readfortran(f->file, buf0, deva_intsize, 1 * pread, pskip,
                                f->swapped);
    buf = buf0;
    assert(nread == pread);
    ...
    for (i = 0; i < pread; i++) {
        ...
        if (deva_realsize == sizeof(int32_t))
            ptype = (int)*((int32_t *)buf);
        else if (deva_realsize == sizeof(int64_t))
            ptype = (int)*((int64_t *)buf);
        buf += deva_intsize;
    ...
    nread =
        io_util_readfortran(f->file, buf0, deva_realsize, 0, pskip, f->swapped);

    /*******************************************************************\
     *  READ particle energies                                         *
    \*******************************************************************/
    nread = io_util_readfortran(f->file, buf0, deva_realsize,
                                (f->header->ngas + f->header->nstar), pskip,
                                f->swapped);
    buf = buf0;
    ...
    for (i = 0; i < nread; i++) {
        ...
        if (isgreaterequal(*((float *)strg.u.val), PGAS))
            // NOTE: we are scaling the thermal energies to km/sec right here,
            // right now!
...
extern uint64_t io_deva_readpart_raw(io_logging_t log, io_deva_t f,
                                     uint64_t pskip, uint64_t pread,
                                     io_file_strg_struct_t strg) {
    uint64_t i, j, psum;
    int ptype;
    ...
    \*******************************************************************/
    bytes_file =
        4; // DEVA file stores every variable (int, float) as a 4 byte number
    ...
    /* Go to the first particle we want to read */
    partsize =
        3 * bytes_file; // 3 x bytes_file for the three position variables
    fseek(f->file, partsize * pskip, SEEK_CUR);
    ...
    /* Go to the first particle we want to read */
    partsize =
        3 * bytes_file; // 3 x bytes_file for the three velocity variables
    fseek(f->file, partsize * pskip, SEEK_CUR);
    ...
    if ((f->header->ngas + f->header->nstar) > pskip) {
        ...
        if ((f->header->ngas + f->header->nstar) > pskip + pread) {
            ...
            for (i = 0; i < pread; i++) {
                ...
                /* Store the energy in the array only for PGAS particle */
                if (isgreaterequal(*((float *)strg.u.val), PGAS))
                    // NOTE: we are scaling the thermal energies to km/sec right
                    // here, right now!
                    *((float *)strg.u.val) = (float)(dummy * e_fac);
        ...
        /* pread also enters into the DM regime that does not have thermal
           energies */
        else {
...
extern uint64_t io_deva_writepart(io_logging_t log, io_deva_t f, uint64_t pskip,
                                  uint64_t pwrite, io_file_strg_struct_t strg) {
    return 0;
}

extern uint64_t io_deva_writepart_ord(io_logging_t log, io_deva_t f,
...
extern bool io_deva_get(io_logging_t log, io_deva_t f, io_file_get_t what,
                        void *res) {
    if ((f == NULL) || (f->header == NULL))
        return false;
    ...
    case IO_FILE_GET_PMASS:
        *((double *)res) = f->header->box100 * f->header->box100 *
                           f->header->box100 * f->header->omega0 * rhoc0 /
                           f->sumweight;
        break;
...
extern bool io_deva_set(io_logging_t log, io_deva_t f, io_file_get_t what,
                        void *res) {
    if ((f == NULL) || (f->header == NULL))
        return false;
...
extern void io_deva_log(io_logging_t log, io_deva_t f) {
    ...
    io_logging_msg(log, INT32_C(5), "  Header size:          %" PRIi8,
...
extern void io_deva_resetscale(io_logging_t log, io_deva_t f, double posscale,
                               double weightscale) {
    if (f == NULL)
        return;
...
extern uint64_t io_deva_scale_particles(io_logging_t log, double maxpos[],
                                        double minpos[], float *boxsize,
                                        float expansion, float h0t0,
                                        double posscale, double mmass,
...
#ifdef WITH_MPI
extern void io_deva_scale_global(io_logging_t log, MPI_Comm comm,
