...
extern io_mcubep3m_t io_mcubep3m_open(io_logging_t log, char *fname,
    ...
    if (io_util_split_pathfname(fname, &(f->path), &(f->stem)) == NULL) {
        io_logging_fatal(log, "Could not split %s in path and filename.",
                         fname);
        free(f);
        return NULL;
    }
    io_logging_msg(log, INT32_C(1), "Will look in %s for %s.", f->path,
                   f->stem);

    /* Get the filenames */
    f->numfiles = io_util_findfiles(f->path, f->stem, "%i", ".dat", &fnames);
    if (f->numfiles <= 0) {
...
#ifdef WITH_MPI
        f->files[i] =
            io_cubep3m_open(log, fnames[i], swapped, mode, f->size + 1);
#else
        f->files[i] = io_cubep3m_open(log, fnames[i], swapped, mode, reader);
#endif
        if ((f->files)[i] == NULL) {
            int32_t j;
...
extern uint64_t io_mcubep3m_readpart(io_logging_t log, io_mcubep3m_t f,
    ...
    /* And do the scaling */
    tmp = io_cubep3m_scale_particles(
        log, f->files[0]->header->boxsize, f->files[0]->header->a,
        f->files[0]->header->lunit, f->files[0]->header->vunit, part_read,
        strg);
    if (tmp != part_read) {
        return tmp;
    }
...
extern uint64_t io_mcubep3m_readpart_raw(io_logging_t log, io_mcubep3m_t f,
    ...
    for (i = 0; i < f->numfiles; i++) {
         ...
         */
        if (io_cubep3m_get(log, f->files[i], IO_FILE_GET_NOPART_IN_FILE,
                           &tmp) != true) {
        ...
        partread = io_cubep3m_readpart_raw(log, f->files[i], pskip_file,
                                           pread_file, strg);
