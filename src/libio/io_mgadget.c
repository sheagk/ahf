...
\**********************************************************************/
extern io_mgadget_t io_mgadget_open_withoutNCPUREADING_EQ_NFILES(
    io_logging_t log, char *fname, io_file_swap_t swapped, io_file_mode_t mode,
    uint32_t reader) {
    int32_t i;
    io_mgadget_t f;
    char **fnames;
    ...
    if (f == NULL) {
        io_logging_memfatal(log,
                            "io_mgdaget_open(): sizeof(io_mgadget_struct_t)");
        return NULL;
    }
    ...
    if (io_util_split_pathfname(fname, &(f->path), &(f->stem)) == NULL) {
        io_logging_fatal(
            log, "io_mgdaget_open(): Could not split %s in path and filename.",
            fname);
        if (f->path)
            free(f->path);
        if (f->stem)
...
#endif
    {
        ...
        if (f->numfiles <= 0) {
            io_logging_fatal(log,
                             "io_mgdaget_open(): Could not open anything "
                             "starting with %s in %s.",
                             f->stem, f->path);
            if (f->path)
                free(f->stem);
            if (f->stem)
    ...
    if (f->numfiles != global_io.params->reader) {
        io_logging_fatal(
            log,
            "io_mgdaget_open(): You are using NCPUREADING_EQ_NFILES and hence "
            "the number of files %d should be the number of MPI tasks reading "
            "%d (size_mycomm=%d).",
    ...
    if (limit.rlim_max < f->numfiles + NFILES_EXTRA) {
        ...
        fprintf(
            stderr,
            "   only the super-user can increase beyond the maximum limit:\n");
        fprintf(stderr, "     numfiles = %d\n", f->numfiles);
        fprintf(
            stderr,
            "     rlim_max = %llu (incl. 3 for stdin, stdout, and stderr)\n",
            limit.rlim_max);
...
#ifdef WITH_MPI
        fprintf(stderr,
                "\nincreasing number of file descriptors on rank %d (of %d) to "
                "%d\n",
...
#ifdef WITH_MPI
            fprintf(stderr,
                    "setrlimit() failed with errno=%d (rank %d of %d)\n", errno,
                    f->rank, f->size);
#else
...
#ifdef WITH_MPI
            fprintf(stderr,
                    "getrlimit() failed with errno=%d (rank %d of %d)\n", errno,
                    f->rank, f->size);
#else
            fprintf(stderr, "getrlimit() failed with errno=%d\n", errno);
#endif
    ...
    if (f->rank_mycomm == 0) {
        ...
        for (i = 0; i < f->numfiles; i++) {
#ifdef DEBUG_MGADGET
            io_logging_msg(
                log, 6, "\ntrying to open file %s (%d of %d) for reading ... ",
                fnames[i], i + 1, f->numfiles);
#endif
             ...
             */
            (f->files)[i] =
                io_gadget_open(log, fnames[i], swapped, mode, f->size + 1);
#else
...
#ifdef FOPENCLOSE
            // fprintf(stderr,"FOPENCLOSE: opened %s ...
            // ",((f->files)[i])->fname);
            fclose(((f->files)[i])->file);
            // DO NOT SET THE POINTER TO NULL AS file!=NULL INDICATES FOR
            // FOLLOWING ROUTINES THAT FILE COULD BE OPENED
            // fprintf(stderr,"and closed file (temporarily)\n");
#endif
        }
#if (defined WITH_MPI && defined BCASTHEADER)
    }
    /* we nevertheless need to generate all the file structures ready to receive
       the broadcast */
    else {
        for (i = 0; i < f->numfiles; i++) {
            ((f->files)[i]) =
                (io_gadget_t)calloc(sizeof(io_gadget_struct_t), 1);
    ...
    for (i = 0; i < f->numfiles; i++) {
        MPI_Bcast((f->files)[i], sizeof(io_gadget_struct_t), MPI_BYTE, 0,
                  f->mycomm);

        /* only rank == 0 properly allocated the memory for the filename and
         * copied the data there */
        if (f->rank_mycomm != 0) {
            ((f->files)[i])->fname =
                (char *)malloc(sizeof(char) * (strlen(fnames[i]) + 1));
...
extern io_mgadget_t io_mgadget_open(io_logging_t log, char *fname,
...
#if (defined WITH_MPI &&                                                       \
     (defined BCASTHEADER ||                                                   \
      defined NCPUREADING_EQ_NFILES)) // even for NCPUREADING_EQ_NFILES only the
                                      // master determines 'numfiles'
    if (f->rank_mycomm == 0)
#endif
    {
        /* Get the filenames */
...
#endif /* (defined WITH_MPI && (defined BCASTHEADER || defined                 \
          NCPUREADING_EQ_FILES)) */
    ...
    for (i = 0; i < f->numfiles; i++) {
        ...
        if (i == f->rank_mycomm) {
            ...
            io_logging_msg(
                log, 6,
                "\nrank %d trying to open file %s (%d of %d) for reading ... ",
                f->rank_mycomm, fnames[i], i + 1, f->numfiles);
#endif
...
#else
            (f->files)[i] =
                io_gadget_open(log, fnames[i], swapped, mode, reader);
#endif
    ...
    for (i = 0; i < f->numfiles; i++) {
        MPI_Bcast((f->files)[i], sizeof(io_gadget_struct_t), MPI_BYTE, i,
                  f->mycomm);

        /* only rank == i properly allocated the memory for the filename and
         * copied the data there */
        if (f->rank_mycomm != i) {
            ((f->files)[i])->fname =
                (char *)malloc(sizeof(char) * (strlen(fnames[i]) + 1));
...
extern void io_mgadget_close(io_logging_t log, io_mgadget_t *f) {
    ...
    /* free() all related structures and fclose() the files (in case the file
     * pointer is not NULL) */
    for (i = 0; i < (*f)->numfiles; i++)
        io_gadget_close(log, &(((*f)->files)[i]));
...
extern void io_mgadget_init(io_logging_t log, io_mgadget_t f) {
    ...
    if (f->rank_mycomm == 0) {
        ...
        for (i = 0; i < f->numfiles; i++) {
            ((f->files)[i])->header =
                (io_gadget_header_t)malloc((size_t)GADGET_HEADER_SIZE + 1);
        }
    }

    /* Broadcast the header of each file from rank==0 to all other MPI tasks */
    for (i = 0; i < f->numfiles; i++) {
        MPI_Bcast(((f->files)[i])->header, ((size_t)GADGET_HEADER_SIZE + 1),
                  MPI_BYTE, 0, f->mycomm);

        /* io_gadget_init() also set some more relevant values in f */
        MPI_Bcast(&(((f->files)[i])->multimass), sizeof(int8_t), MPI_BYTE, 0,
                  f->mycomm);
        MPI_Bcast(&(((f->files)[i])->no_part), sizeof(uint64_t), MPI_BYTE, 0,
                  f->mycomm);
        MPI_Bcast(&(((f->files)[i])->no_part_with_mass), sizeof(uint64_t),
                  MPI_BYTE, 0, f->mycomm);
...
extern uint64_t io_mgadget_readpart(io_logging_t log, io_mgadget_t f,
    ...
    tmp = io_gadget_scale_particles(
        log, f->maxpos, f->minpos, &(f->files[0]->header->boxsize),
        f->files[0]->header->expansion, f->posscale, f->mmass, part_read, strg);
...
extern uint64_t io_mgadget_readpart_raw(io_logging_t log, io_mgadget_t f,
    ...
    for (i = 0; i < f->numfiles; i++) {
        ...
        if (io_gadget_get(log, f->files[i], IO_FILE_GET_NOPART, &tmp) != true) {
            io_logging_fatal(log, "Could not get number of particles from %s",
                             f->files[i]->fname);
        }
        ...
        if (i == f->rank_mycomm) {
            ...
            partread = io_gadget_readpart_raw(log, f->files[i], pskip_file,
                                              pread_file, strg);

            if (pread_file != partread) {
                io_logging_fatal(log,
                                 "Something went wrong. Wanted to read %" PRIu64
                                 " particles, but got %" PRIu64 ". Aborting.",
                                 pread_file, partread);
            ...
            strg.posx.val =
                (void *)(((char *)strg.posx.val) + strg.posx.stride * partread);
            strg.posy.val =
                (void *)(((char *)strg.posy.val) + strg.posy.stride * partread);
            strg.posz.val =
                (void *)(((char *)strg.posz.val) + strg.posz.stride * partread);
            strg.momx.val =
                (void *)(((char *)strg.momx.val) + strg.momx.stride * partread);
            strg.momy.val =
                (void *)(((char *)strg.momy.val) + strg.momy.stride * partread);
            strg.momz.val =
                (void *)(((char *)strg.momz.val) + strg.momz.stride * partread);
            if (strg.weight.val != NULL)
                strg.weight.val = (void *)(((char *)strg.weight.val) +
                                           strg.weight.stride * partread);
            if (strg.id.val != NULL)
                strg.id.val =
                    (void *)(((char *)strg.id.val) + strg.id.stride * partread);
            if (strg.u.val != NULL)
                strg.u.val =
                    (void *)(((char *)strg.u.val) + strg.u.stride * partread);
#ifdef METALHACK
            if (strg.z.val != NULL)
                strg.z.val =
                    (void *)(((char *)strg.z.val) + strg.z.stride * partread);
            if (strg.age.val != NULL)
                strg.age.val = (void *)(((char *)strg.age.val) +
                                        strg.age.stride * partread);
#endif

            /* Update the important things needed for scaling */
            f->sumweight =
                f->files[i]->sumweight; // no accumulation of sumweight as we do
                                        // an MPI_SUM reduction below
        ...
        if (strg.age.val != NULL)
            strg.age.val =
                (void *)(((char *)strg.age.val) + strg.age.stride * partread);
#endif
...
#if (defined WITH_MPI && defined NCPUREADING_EQ_NFILES)
    {
        ...
        MPI_Allreduce(&(f->minweight), buffer, 1, MPI_DOUBLE, MPI_MIN,
                      f->mycomm);
        f->minweight = buffer[0];

        MPI_Allreduce(&(f->maxweight), buffer, 1, MPI_DOUBLE, MPI_MAX,
                      f->mycomm);
        f->maxweight = buffer[0];

        MPI_Allreduce(&(f->sumweight), buffer, 1, MPI_DOUBLE, MPI_SUM,
                      f->mycomm);
        f->sumweight = buffer[0];

        /* note, this reduction is in fact repeated by io_gadget_scale_global()
         * but what the heck... */
...
extern bool io_mgadget_get(io_logging_t log, io_mgadget_t f, io_file_get_t what,
    ...
    case IO_FILE_GET_NOSPECIES:
        if (isgreater(f->files[0]->header->massarr[1], 0.0))
            ...
            *((int *)res) = 1;
            io_logging_warn(log, INT32_C(1), "Not implemented yet.");
            return false;
        }
        break;
    case IO_FILE_GET_BOXSIZE:
        *((double *)res) = f->files[0]->header->boxsize * f->posscale;
        break;
...
extern void io_mgadget_log(io_logging_t log, io_mgadget_t f) {
    ...
    io_logging_msg(log, INT32_C(5), "  Path:                %s", f->path);
    io_logging_msg(log, INT32_C(5), "  Stem:                %s", f->stem);
    io_logging_msg(log, INT32_C(5), "  Number of files:     %" PRIi32,
                   f->numfiles);
    io_logging_msg(log, INT32_C(5), "  Number of particles: %" PRIu64,
                   f->no_part);
    io_logging_msg(log, INT32_C(5), "  Multimass         :  %" PRIi8,
                   f->multimass);
    io_logging_msg(log, INT32_C(5), "  Mmass             :  %g", f->mmass);
    io_logging_msg(log, INT32_C(5), "  Minimal Weight    :  %g", f->minweight);
    io_logging_msg(log, INT32_C(5), "  Maximal Weight    :  %g", f->maxweight);
    io_logging_msg(log, INT32_C(5), "  Sum of weights    :  %g", f->sumweight);

    for (i = 0; i < f->numfiles; i++) {
        io_logging_msg(log, INT32_C(5), "  ---> File %" PRIi32 ":", i);
