...
extern io_tipsy_header_t io_tipsy_header_get(io_logging_t log, io_tipsy_t f) {
    ...
    if ((ftipsy = fopen("tipsy.info", "r")) == NULL) {
        io_logging_fatal(
            log,
            "Could not open tipsy.info containing the following information:");
        io_logging_fatal(log, "0.3                     TIPSY_OMEGA0");
        io_logging_fatal(log, "0.7                     TIPSY_LAMBDA0");
        io_logging_fatal(
            log, "20.0                    TIPSY_BOXSIZE           [Mpc/h]");
        io_logging_fatal(
            log, "690.988298942671        TIPSY_VUNIT             [km/sec]");
        io_logging_fatal(
            log, "2.2197e15               TIPSY_MUNIT             [Msun/h]");
        io_logging_fatal(
            log,
            "2.2197e15               TIPSY_EUNIT             [(km/sec)^2]");
    ...
    io_logging_msg(log, INT32_C(5),
                   ...
                   dummy->omega0);
    io_logging_msg(log, INT32_C(5),
                   "   tipsy.info -> lambda0:                        %e",
                   dummy->lambda0);
    io_logging_msg(log, INT32_C(5),
                   "   tipsy.info -> boxsize:                        %e",
                   dummy->boxsize);
    io_logging_msg(log, INT32_C(5),
                   "   tipsy.info -> vunit:                          %e",
                   dummy->vunit);
...
extern void io_tipsy_header_log(io_logging_t log, io_tipsy_header_t header) {
    io_logging_msg(log, INT32_C(5), "   time:                         %e",
                   header->time);
    io_logging_msg(log, INT32_C(5), "  nbodies:                       %" PRIu32,
                   header->nbodies);
    io_logging_msg(log, INT32_C(5), "  ndim:                          %" PRIu32,
                   header->ndim);
    io_logging_msg(log, INT32_C(5), "  nsph:                          %" PRIu32,
                   header->nsph);
    io_logging_msg(log, INT32_C(5), "  ndark:                         %" PRIu32,
                   header->ndark);
    io_logging_msg(log, INT32_C(5), "  nstar:                         %" PRIu32,
                   header->nstar);
    io_logging_msg(log, INT32_C(5), "  pad:                           %" PRIu32,
                   header->pad);
    return;
}
