 ...
 */
inline static io_amiga_t local_openopen(io_logging_t log, io_amiga_t f,
                                        io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_amiga_t f,
 ...
 * \return Nothing.
 */
inline static void local_openfsol(io_logging_t log, io_amiga_t f);
 ...
 */
inline static int32_t local_write_common(io_logging_t log, io_amiga_t f,
                                         void *weight, uint64_t pskip,
                                         int32_t bytes);
...
#define INCR(a, b) (a) = (void *)(((char *)(a)) + (b))
...
extern uint64_t io_amiga_readpart(io_logging_t log, io_amiga_t f,
    ...
    if ((f == NULL) || (f->header == NULL) || (f->header->no_part <= 0) ||
...
#define show(bf, bs, what)                                                     \
    {                                                                          \
        if (bf > bs) {                                                         \
            io_logging_warn(log, INT32_C(3), "   " what " (read, downcast)");  \
        } else if (bf == bs) {                                                 \
            io_logging_msg(log, INT32_C(3), "   " what " (read)");             \
        } else {                                                               \
            io_logging_warn(log, INT32_C(3), "   " what " (read, upcast)");    \
        }                                                                      \
    }
    ...
    if (strg.id.val != NULL)
        io_logging_warn(log, INT32_C(3), "   id     (set to position in file)");
    if (strg.u.val != NULL)
...
#undef show
     ...
     */
    fseek(f->file,
          (long)4 + (long)(AMIGA_HEADER_SIZE) + (long)(pskip * partsize) +
    ...
    io_logging_msg(log, INT32_C(3),
                   "Done with reading %" PRIu64 " particles from %s", i,
                   f->fname);
...
extern uint64_t io_amiga_writepart(io_logging_t log, io_amiga_t f,
    ...
    if (local_write_common(log, f, strg.weight.val, pskip, strg.bytes_float) ==
        0)
        return UINT64_C(0);
    ...
    /* Update logfile */
    io_logging_msg(log, INT32_C(3), "Wrote %" PRIu64 " particles to file",
                   no_part);
...
extern uint64_t io_amiga_writepart_ord(io_logging_t log, io_amiga_t f,
    ...
    } while ((*((char **)nxt_part) != NULL) && (no_part < pwrite));
...
extern bool io_amiga_get(io_logging_t log, io_amiga_t f, io_file_get_t what,
    ...
    case IO_FILE_GET_OMEGAL:
        *((double *)res) = f->header->lambda0;
        break;
    case IO_FILE_GET_H:
        io_logging_fatal(log, "The Hubble parameter is not available "
                              "for AMIGA files.");
        return false;
...
extern void io_amiga_log(io_logging_t log, io_amiga_t f) {
    ...
    io_logging_msg(log, INT32_C(5), "  Swapping:          %" PRIi8, f->swapped);
    io_logging_msg(log, INT32_C(5), "  File sizeof(long): %" PRIi8,
                   f->file_sizeof_long);
    io_logging_msg(log, INT32_C(5), "  minweight:         %g", f->minweight);
    io_logging_msg(log, INT32_C(5), "  maxweight:         %g", f->maxweight);
...
inline static void local_openswapped(io_logging_t log, io_amiga_t f,
    ...
    if (f->mode == IO_FILE_READ) {
        ...
        default:
            io_logging_msg(log, INT32_C(3), "Will find out swap status");
        }
    }
...
inline static int32_t local_write_common(io_logging_t log, io_amiga_t f,
    ...
    if ((uint64_t)ftell(f->file) <
        sizeof(io_amiga_header_struct_t) + sizeof(uint32_t)) {
        int8_t nix = 0;
        int32_t i;

        io_logging_msg(log, INT32_C(3), "Need to write a dummy header");
        rewind(f->file);
        fwrite((void *)&(f->file_sizeof_long), sizeof(uint32_t), 1, f->file);
        for (i = 0; i < AMIGA_HEADER_SIZE; i++)
            fwrite((void *)&nix, sizeof(int8_t), 1, f->file);
        io_logging_msg(log, INT32_C(3), "Wrote a dummy header");
    }
    ...
    if (weight != NULL)
        partsize += bytes;
    fseek(f->file,
          4L + (long)(sizeof(io_amiga_header_struct_t)) + (pskip * partsize),
          SEEK_SET);
