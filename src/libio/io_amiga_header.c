 ...
 */
inline static void local_getreadlong(io_logging_t log, io_amiga_t f,
                                     long *trgt);

/**********************************************************************\
 *    Implementation of global functions                              *
\**********************************************************************/
extern io_amiga_header_t io_amiga_header_get(io_logging_t log, io_amiga_t f) {
    ...
    io_util_readstring(f->file, dummy->header, AMIGA_HEADER_HEADERSTRING);
    ...
    io_util_readstring(f->file, dummy->dummy,
...
extern io_amiga_header_t io_amiga_header_new(io_logging_t log) {
    io_amiga_header_t dummy;

    dummy =
        (io_amiga_header_t)calloc(sizeof(io_amiga_header_struct_t), (size_t)1);
    if (dummy == NULL) {
        io_logging_memfatal(log, "AMIGA header structure");
        return NULL;
    }
...
extern void io_amiga_header_del(io_logging_t log, io_amiga_header_t *header) {
...
extern void io_amiga_header_write(io_logging_t log, io_amiga_header_t header,
                                  io_amiga_t f) {
    int32_t tmp;
...
extern void io_amiga_header_log(io_logging_t log, io_amiga_header_t header) {
    io_logging_msg(log, INT32_C(5), "Headerobject information:");
    io_logging_msg(log, INT32_C(5), "  Header:                      %s",
                   header->header);
    io_logging_msg(log, INT32_C(5), "  Multimass:                   %" PRIi32,
                   header->multi_mass);
    io_logging_msg(log, INT32_C(5), "  Double precision:            %" PRIi32,
                   header->double_precision);
    io_logging_msg(log, INT32_C(5), "  Number of particles:         %li",
                   header->no_part);
    io_logging_msg(log, INT32_C(5), "  Number of mass species:      %li",
                   header->no_species);
    io_logging_msg(log, INT32_C(5), "  Number of virtual particles: %e",
                   header->no_vpart);
    io_logging_msg(log, INT32_C(5), "  Timestep                   : %e",
                   header->timestep);
    io_logging_msg(log, INT32_C(5), "  Number of timestep         : %" PRIi32,
                   header->no_timestep);
    io_logging_msg(log, INT32_C(5), "  Boxsize                    : %e",
                   header->boxsize);
    io_logging_msg(log, INT32_C(5), "  Omega0                     : %e",
                   header->omega0);
    io_logging_msg(log, INT32_C(5), "  lambda0                    : %e",
                   header->lambda0);
    io_logging_msg(log, INT32_C(5), "  pmass                      : %e",
                   header->pmass);
    io_logging_msg(log, INT32_C(5), "  cur_reflevel               : %e",
                   header->cur_reflevel);
    io_logging_msg(log, INT32_C(5), "  cur_frcres                 : %e",
                   header->cur_frcres);
    io_logging_msg(log, INT32_C(5), "  a_initial                  : %e",
                   header->a_initial);
    io_logging_msg(log, INT32_C(5), "  a_current                  : %e",
                   header->a_current);
    io_logging_msg(log, INT32_C(5), "  K_initial                  : %e",
                   header->K_initial);
    io_logging_msg(log, INT32_C(5), "  K_current                  : %e",
                   header->K_current);
    io_logging_msg(log, INT32_C(5), "  U_initial                  : %e",
                   header->U_initial);
    io_logging_msg(log, INT32_C(5), "  U_current                  : %e",
                   header->U_current);
    io_logging_msg(log, INT32_C(5), "  Eintegral                  : %e",
                   header->Eintegral);
    io_logging_msg(log, INT32_C(5), "  Econst                     : %e",
                   header->Econst);
    io_logging_msg(log, INT32_C(5), "  paramNSTEPS                : %e",
                   header->paramNSTEPS);
    io_logging_msg(log, INT32_C(5), "  paramNGRID_DOM             : %e",
                   header->paramNGRID_DOM);
    io_logging_msg(log, INT32_C(5), "  paramNth_dom               : %e",
                   header->paramNth_dom);
    io_logging_msg(log, INT32_C(5), "  paramNth_ref               : %e",
                   header->paramNth_ref);
    io_logging_msg(log, INT32_C(5), "  paramE_UPDATE              : %e",
                   header->paramE_UPDATE);
    io_logging_msg(log, INT32_C(5), "  paramCELLFRAC_MAX          : %e",
                   header->paramCELLFRAC_MAX);
    io_logging_msg(log, INT32_C(5), "  paramCELLFRAC_MIN          : %e",
                   header->paramCELLFRAC_MIN);
    io_logging_msg(log, INT32_C(5), "  paramCA_CRIT               : %e",
                   header->paramCA_CRIT);
    io_logging_msg(log, INT32_C(5), "  paramMAX_L1DIM             : %e",
                   header->paramMAX_L1DIM);
    io_logging_msg(log, INT32_C(5), "  paramDOMSWEEPS             : %e",
                   header->paramDOMSWEEPS);
    io_logging_msg(log, INT32_C(5), "  paramREFSWEEPS             : %e",
                   header->paramREFSWEEPS);
    io_logging_msg(log, INT32_C(5), "  paramAHF_MINPART           : %e",
                   header->paramAHF_MINPART);
    io_logging_msg(log, INT32_C(5), "  paramAHF_VTUNE             : %e",
                   header->paramAHF_VTUNE);
    io_logging_msg(log, INT32_C(5), "  paramAHF_RISE              : %e",
                   header->paramAHF_RISE);
    io_logging_msg(log, INT32_C(5), "  paramAHF_SLOPE             : %e",
                   header->paramAHF_SLOPE);
    io_logging_msg(log, INT32_C(5), "  paramAHF_MAXNRISE          : %e",
                   header->paramAHF_MAXNRISE);
    io_logging_msg(log, INT32_C(5), "  min_weight                 : %e",
                   header->min_weight);
    io_logging_msg(log, INT32_C(5), "  max_weight                 : %e",
                   header->max_weight);
    io_logging_msg(log, INT32_C(5), "  t_unit                     : %e",
                   header->t_unit);
    io_logging_msg(log, INT32_C(5), "  B_init                     : %e",
                   header->B_init);
    io_logging_msg(log, INT32_C(5), "  param_dummy5               : %e",
                   header->param_dummy5);
    io_logging_msg(log, INT32_C(5), "  param_dummy6               : %e",
                   header->param_dummy6);
    io_logging_msg(log, INT32_C(5), "  param_dummy7               : %e",
                   header->param_dummy7);
    io_logging_msg(log, INT32_C(5), "  param_dummy8               : %e",
                   header->param_dummy8);
    io_logging_msg(log, INT32_C(5), "  version                    : %e",
                   header->version);
    io_logging_msg(log, INT32_C(5), "  build                      : %" PRIi32,
                   header->build);
