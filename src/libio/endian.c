 ...
 *          size.
 */
static uint32_t local_findBlockEnd(uint32_t b1, uint32_t b1Swapped, FILE *f);
 ...
 */
static endian_t local_decideOnFileEndian(uint32_t b1, uint32_t b1Swapped,
                                         uint32_t b2);
...
static endian_t local_decideOnFileEndian(uint32_t b1, uint32_t b1Swapped,
    ...
    if (b1 == b2)
        ...
        fileEndianess =
            (systemEndianess == ENDIAN_BIG) ? ENDIAN_LITTLE : ENDIAN_BIG;
    else
        diediedie(EXIT_FAILURE);
