...
inline static io_parameter_t local_allocStruct(void);

inline static parse_ini_t local_getIni(const char *fname);

inline static void local_initStruct(io_parameter_t params);

static void local_readRequired(io_parameter_t params, parse_ini_t ini);

static void local_printStruct(FILE *f, const io_parameter_t params);
...
inline static parse_ini_t local_getIni(const char *fname) {
    ...
    if (ini == NULL) {
        fprintf(stderr, "FATAL: Could not open parameter file (%s)\n",
...
static void local_readRequired(io_parameter_t params, parse_ini_t ini) {
    ...
    getFromIni(&(params->icfile_name), parse_ini_get_string, ini, "ic_filename",
               local_secName);

    /* Get the type of the input file */
    getFromIni(&tmpInt, parse_ini_get_int32, ini, "ic_filetype", local_secName);
    params->ic_filetype = (io_file_type_t)tmpInt;

    /* Get the outfile prefix */
    getFromIni(&(params->outfile_prefix), parse_ini_get_string, ini,
               "outfile_prefix", local_secName);

    /* Get the domain grid size */
    getFromIni(&(params->NGRID_DOM), parse_ini_get_int32, ini, "LgridDomain",
               local_secName);

    /* Get the maximum grid size */
    getFromIni(&(params->NGRID_MAX), parse_ini_get_int32, ini, "LgridMax",
               local_secName);

    /* Get the refinment criterion for the domain grid */
    getFromIni(&(params->Nth_dom), parse_ini_get_double, ini, "NperDomCell",
               local_secName);

    /* Get the refinement criterion for the refinment grids */
    getFromIni(&(params->Nth_ref), parse_ini_get_double, ini, "NperRefCell",
               local_secName);

    /* Shall we use rho_back or rho_crit for halo edge */
    getFromIni(&(params->UseRhoBack), parse_ini_get_int32, ini, "RhoVir",
               local_secName);

    /* Get virial overdensity criterion */
    getFromIni(&(params->UserDvir), parse_ini_get_double, ini, "Dvir",
               local_secName);

    /* Get the maximum gather radius */
    getFromIni(&(params->MaxGatherRad), parse_ini_get_double, ini,
               "MaxGatherRad", local_secName);

    /* Get the tune parameter for the escape velocity */
    getFromIni(&(params->AHF_VTUNE), parse_ini_get_double, ini, "VescTune",
               local_secName);

    /* Get the maximum gather radius */
    getFromIni(&(params->AHF_MINPART), parse_ini_get_int32, ini, "NminPerHalo",
               local_secName);

#ifdef WITH_MPI
    /* Get the number of readers */
    getFromIni(&(params->reader), parse_ini_get_uint32, ini, "NcpuReading",
               local_secName);
    /* Get the LOADBALANCE_DOMAIN_GRID */
    getFromIni(&(params->lb_level), parse_ini_get_int32, ini,
               "LevelDomainDecomp", local_secName);
#else
    params->reader = 1;
    params->lb_level = 0;
#endif
    ...
    getFromIni(&(params->lrsi_beta), parse_ini_get_double, ini, "lrsi_beta",
               local_secName);
    getFromIni(&(params->lrsi_r_s), parse_ini_get_double, ini, "lrsi_r_s",
               local_secName);
#endif

#if (defined AHFmixHaloIDandSnapID || defined SUSSING2013)
    getFromIni(&(params->isnap), parse_ini_get_uint64, ini, "snapID",
               local_secName);
#endif

#ifdef DARK_ENERGY
    /* Get name of file containing the dark energy relevant tables */
    getFromIni(&(params->defile_name), parse_ini_get_string, ini, "de_filename",
               local_secName);
#endif

    /* read additional information for some input file formats and write the
     * respective IO_FILE.info */

    /*-----------------
     *     GADGET
     *-----------------*/
    if (params->ic_filetype == IO_FILE_GADGET ||
        params->ic_filetype == IO_FILE_MGADGET) {
        /* Get GADGET_MUNIT */
        getFromIni(&(params->GADGET_m2Msunh), parse_ini_get_double, ini,
                   "GADGET_MUNIT", "GADGET");
        /* Get GADGET_LUNIT */
        getFromIni(&(params->GADGET_l2Mpch), parse_ini_get_double, ini,
                   "GADGET_LUNIT", "GADGET");
    ...
    if (params->ic_filetype == IO_FILE_ART) {
        ...
        getFromIni(&ART_BOXSIZE, parse_ini_get_double, ini, "ART_BOXSIZE",
                   "ART");
        getFromIni(&ART_MUNIT, parse_ini_get_double, ini, "ART_MUNIT", "ART");
    ...
    if (params->ic_filetype == IO_FILE_TIPSY) {
        FILE *fpout;
        double TIPSY_BOXSIZE, TIPSY_MUNIT, TIPSY_VUNIT, TIPSY_EUNIT,
            TIPSY_OMEGA0, TIPSY_LAMBDA0;

        getFromIni(&TIPSY_BOXSIZE, parse_ini_get_double, ini, "TIPSY_BOXSIZE",
                   "TIPSY");
        getFromIni(&TIPSY_MUNIT, parse_ini_get_double, ini, "TIPSY_MUNIT",
                   "TIPSY");
        getFromIni(&TIPSY_VUNIT, parse_ini_get_double, ini, "TIPSY_VUNIT",
                   "TIPSY");
        getFromIni(&TIPSY_EUNIT, parse_ini_get_double, ini, "TIPSY_EUNIT",
                   "TIPSY");
        getFromIni(&TIPSY_OMEGA0, parse_ini_get_double, ini, "TIPSY_OMEGA0",
                   "TIPSY");
        getFromIni(&TIPSY_LAMBDA0, parse_ini_get_double, ini, "TIPSY_LAMBDA0",
                   "TIPSY");
     ...
     *    CUBEP3M
     *-----------------*/
    if (params->ic_filetype == IO_FILE_CUBEP3M ||
        params->ic_filetype == IO_FILE_MCUBEP3M) {
        FILE *fpout;
        double CUBEP3M_BOXSIZE, CUBEP3M_NGRID, CUBEP3M_NODES_DIM,
            CUBEP3M_OMEGA0, CUBEP3M_LAMBDA0;

        getFromIni(&CUBEP3M_BOXSIZE, parse_ini_get_double, ini,
                   "CUBEP3M_BOXSIZE", "CUBEP3M");
        getFromIni(&CUBEP3M_NGRID, parse_ini_get_double, ini, "CUBEP3M_NGRID",
                   "CUBEP3M");
        getFromIni(&CUBEP3M_NODES_DIM, parse_ini_get_double, ini,
                   "CUBEP3M_NODES_DIM", "CUBEP3M");
        getFromIni(&CUBEP3M_OMEGA0, parse_ini_get_double, ini, "CUBEP3M_OMEGA0",
                   "CUBEP3M");
        getFromIni(&CUBEP3M_LAMBDA0, parse_ini_get_double, ini,
                   "CUBEP3M_LAMBDA0", "CUBEP3M");
...
static void local_printStruct(FILE *f, const io_parameter_t params) {
    ...
    if (global_mpi.rank == 0) {
        ...
        fprintf(f, "\n");
        fprintf(f, "These are the input parameters you provided, please check "
                   "them carefully again:\n");
        fprintf(f, "==========================================================="
                   "====================\n");
