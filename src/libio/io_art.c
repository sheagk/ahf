 ...
 */
inline static io_art_t local_openopen(io_logging_t log, io_art_t f,
                                      io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_art_t f,
 ...
 */
inline static io_art_t local_opengetswap(io_logging_t log, io_art_t f);
 ...
 * \return
 */
inline static int32_t local_write_common(io_logging_t log, io_art_t f,
                                         uint64_t pskip, int32_t bytes);

/**********************************************************************\
 *    Implementation of global functions                              *
\**********************************************************************/
extern io_art_t io_art_open(io_logging_t log, char *fname,
...
extern void io_art_close(io_logging_t log, io_art_t *f) {
    /* Catch NULLs */
    if (f == NULL || *f == NULL)
        return;
...
extern void io_art_init(io_logging_t log, io_art_t f) {
    if (f == NULL)
        return;
...
extern uint64_t io_art_readpart(io_logging_t log, io_art_t f, uint64_t pskip,
    ...
    /* And do the scaling */
#ifdef WITH_MPI
    io_art_scale_global(log, f->mycomm, f->maxpos, f->minpos, &(f->mmass),
                        &(f->minweight), &(f->maxweight), &(f->sumweight));
#endif

    tmp = io_art_scale_particles(
        log, f->maxpos, f->minpos, f->header->boxsize, f->minweight,
        f->header->aexpn, (double)f->header->ngridc, particles_read, strg);
...
extern uint64_t io_art_readpart_raw(io_logging_t log, io_art_t f,
    ...
    \*******************************************************************/
    bytes_file =
        4; // ART file stores every variable (int, float) as a 4 byte number
    ...
    while (preadIROW > 0 && IROW <= Npages) {
        ...
        for (i = 0; i < preadIROW; i++) {
            ...
            fposx = (double)(dummy - 1.0); // positions are in range [1,ngridc]
                                           // and hence subtract 1
            *((float *)strg.posx.val) = (float)fposx;
#ifdef DEBUG_ART
            if (fposx > NGRID || fposx < 0)
                fprintf(stderr, "page %d => x=%f\n", IROW,
                        *((float *)strg.posx.val));
#endif
        ...
        fseek(f->file, (NPAGE - (pskip + preadIROW)) * bytes_file,
              SEEK_CUR); // even the last PAGE is of total length NPAGE

        /* Y */
        fseek(f->file, pskip * bytes_file, SEEK_CUR);
        for (i = 0; i < preadIROW; i++) {
            io_util_readfloat(f->file, &dummy, f->swapped);
            fposy = (double)(dummy - 1.0); // positions are in range [1,ngridc]
                                           // and hence subtract 1
            *((float *)strg.posy.val) = (float)fposy;
#ifdef DEBUG_ART
            if (fposy > NGRID || fposy < 0)
                fprintf(stderr, "page %d => y=%f\n", IROW,
                        *((float *)strg.posy.val));
#endif
        ...
        for (i = 0; i < preadIROW; i++) {
            io_util_readfloat(f->file, &dummy, f->swapped);
            fposz = (double)(dummy - 1.0); // positions are in range [1,ngridc]
                                           // and hence subtract 1
            *((float *)strg.posz.val) = (float)fposz;
#ifdef DEBUG_ART
            if (fposz > NGRID || fposz < 0)
                fprintf(stderr, "page %d => z=%f\n", IROW,
                        *((float *)strg.posz.val));
#endif
        ...
        if (strg.id.val != NULL) {
            for (i = 0; i < preadIROW; i++) {
                ...
                *((uint32_t *)strg.id.val) =
                    (uint32_t)(Ipart); // use position in file as ID
        ...
        for (i = 0; i < preadIROW; i++) {
            ...
            *((float *)strg.weight.val) = (float)fweight;
            ...
            /* Keep track of total mass (in internal units) and various other
...
extern uint64_t io_art_writepart(io_logging_t log, io_art_t f, uint64_t pskip,
                                 uint64_t pwrite, io_file_strg_struct_t strg) {
    return 0;
}

extern uint64_t io_art_writepart_ord(io_logging_t log, io_art_t f,
...
extern bool io_art_get(io_logging_t log, io_art_t f, io_file_get_t what,
                       void *res) {
    if ((f == NULL) || (f->header == NULL))
        return false;
...
extern void io_art_log(io_logging_t log, io_art_t f) {
...
extern void io_art_resetscale(io_logging_t log, io_art_t f, double posscale,
                              double weightscale) {
    if (f == NULL)
        return;
...
extern uint64_t io_art_scale_particles(io_logging_t log, double maxpos[],
                                       double minpos[], double boxsize,
                                       double minweight, double expansion,
                                       double ngridc, uint64_t particles_read,
                                       io_file_strg_struct_t strg) {
...
#ifdef WITH_MPI
extern void io_art_scale_global(io_logging_t log, MPI_Comm comm, double *maxpos,
                                double *minpos, double *mmass,
