...
extern io_ares_header_t io_ares_header_new(io_logging_t log) {
    ...
    dummy =
        (io_ares_header_t)calloc(sizeof(io_ares_header_struct_t), (size_t)1);
    if (dummy == NULL) {
        io_logging_memfatal(log, "ARES header structure");
        return NULL;
    }
...
extern void io_ares_header_log(io_logging_t log, io_ares_header_t header) {
    ...
    io_logging_msg(log, INT32_C(5), "  Length of identifier       : %" PRIu32,
                   header->len_id);
    io_logging_msg(log, INT32_C(5), "  Identifier                 : %s",
                   header->id);
    io_logging_msg(log, INT32_C(5), "  Number of particles        : %" PRIu64,
                   header->no_part);
    io_logging_msg(log, INT32_C(5), "  Number of particles in file: %" PRIu64,
                   header->no_part_in_file);
    io_logging_msg(log, INT32_C(5), "  Bytes per float in file    : %" PRIu32,
                   header->bytes_float);
    io_logging_msg(log, INT32_C(5), "  Bytes per integer in file  : %" PRIu32,
                   header->bytes_int);
    io_logging_msg(log, INT32_C(5), "  Number of species          : %" PRIu64,
                   header->no_species);
    io_logging_msg(log, INT32_C(5), "  Has mass                   : %" PRIu64,
                   header->has_mass);
    io_logging_msg(log, INT32_C(5), "  Has gas                    : %" PRIu64,
                   header->has_gas);
    ...
    io_logging_msg(log, INT32_C(5), "  pmass                      : %e",
                   header->pmass);
    io_logging_msg(log, INT32_C(5), "  Minimal weight             : %e",
                   header->minweight);
    io_logging_msg(log, INT32_C(5), "  Maximal weight             : %e",
                   header->maxweight);
    ...
    io_logging_msg(log, INT32_C(5), "  Timestep                   : %e",
                   header->timestep);
    io_logging_msg(log, INT32_C(5), "  Minimal SFC key            : %" PRIu64,
                   header->minkey);
    io_logging_msg(log, INT32_C(5), "  Maximal SFC key            : %" PRIu64,
                   header->maxkey);
    io_logging_msg(log, INT32_C(5), "  Loadbalancing level        : %i",
                   header->lb_level);
    io_logging_msg(log, INT32_C(5), "  Rank of writing process    : %i",
                   header->rank);
    io_logging_msg(log, INT32_C(5), "  Size of MPI domain         : %i",
                   header->size);
