 ...
 */
static void local_nullHeader(artHeader_t header);
 ...
 * @return  Returns nothing.
 */
static void local_readHeaderActual(artHeader_t header, FILE *f);
 ...
 * @return  Returns nothing.
 */
static void local_byteswapHeader(artHeader_t header);
 ...
 * @return  Returns nothing.
 */
static void local_calcFactorWeight(artHeader_t header);
 ...
 * @return  Returns nothing.
 */
static void local_calcFactorPosition(artHeader_t header);
 ...
 * @return  Returns nothing.
 */
static void local_calFactorVelocity(artHeader_t header);
...
extern void artHeader_setHeaderString(artHeader_t header, const char *string) {
    ...
    len = strlen(string);
    len = (len > ARTHEADER_HEADERSTRING_LENGTH) ? ARTHEADER_HEADERSTRING_LENGTH
                                                : len;
...
extern void artHeader_write(artHeader_t header, const char *headerFileName) {
    ...
    xfwrite(header->headerString, sizeof(char), ARTHEADER_HEADERSTRING_LENGTH,
            f);
...
static void local_readHeaderActual(artHeader_t header, FILE *f) {
    ...
    xfread(header->headerString, sizeof(char), ARTHEADER_HEADERSTRING_LENGTH,
           f);
...
static void local_calFactorVelocity(artHeader_t header) {
    ...
    header->factorFileVelocityToKms =
        100. * boxsize / (header->ngridc * header->aexpn);
}
