...
\**********************************************************************/
extern io_gadget_header_t io_gadget_header_get(io_logging_t log,
                                               io_gadget_t f) {
...
#ifdef FOPENCLOSE
    // fprintf(stderr,"FOPENCLOSE: reading header information from %s ...
    // ",f->fname);
    fpin = fopen(f->fname, IO_FILE_MODE_READ);
    if (fpin == NULL) {
        io_logging_fatal(
            log, "io_gadget_header_get(): could not open file %s for reading",
            f->fname);
        return NULL;
    }
    ...
    dummy =
        (io_gadget_header_t)malloc((size_t)GADGET_HEADER_SIZE +
                                   1); // make +1 larger because of trailing
                                       // '\0' inserted by io_util_readstring()
    if (dummy == NULL) {
        io_logging_memfatal(log, "Gadget header structure %d",
                            GADGET_HEADER_SIZE + 1);
        return NULL;
    }
    ...
    if (f->ver == 2)
        skipsize +=
            (sizeof(int) * 3 +
             4 * sizeof(char)); // TODO-GADGET: hardwired 4*sizeof(char) value!
...
extern void io_gadget_header_log(io_logging_t log, io_gadget_header_t header) {
    ...
    io_logging_msg(log, INT32_C(5), "  No of particles in file:       %" PRIi32,
                   header->np[0]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIi32,
                   header->np[1]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIi32,
                   header->np[2]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIi32,
    ...
    io_logging_msg(log, INT32_C(5), "                                 %" PRIi32,
                   header->np[5]);
    io_logging_msg(log, INT32_C(5), "  Mass of particle species:      %e",
                   header->massarr[0]);
    io_logging_msg(log, INT32_C(5), "                                 %e",
                   header->massarr[1]);
    io_logging_msg(log, INT32_C(5), "                                 %e",
                   header->massarr[2]);
    io_logging_msg(log, INT32_C(5), "                                 %e",
    ...
    io_logging_msg(log, INT32_C(5), "                                 %e",
                   header->massarr[5]);
    io_logging_msg(log, INT32_C(5), "  Expansion:                     %e",
                   header->expansion);
    io_logging_msg(log, INT32_C(5), "  Redshift:                      %e",
                   header->redshift);
    io_logging_msg(log, INT32_C(5), "  Flagsfr:                       %" PRIi32,
                   header->flagsfr);
    io_logging_msg(log, INT32_C(5), "  Flag Feedback:                 %" PRIi32,
                   header->flagfeedback);
    io_logging_msg(log, INT32_C(5), "  No of particles in total:      %" PRIu32,
                   header->nall[0]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
                   header->nall[1]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
                   header->nall[2]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
    ...
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
                   header->nall[5]);
    io_logging_msg(log, INT32_C(5), "  Flag Cooling:                  %" PRIi32,
                   header->flagcooling);
    ...
    io_logging_msg(log, INT32_C(5), "  Omega0:                        %e",
                   header->omega0);
    io_logging_msg(log, INT32_C(5), "  OmegaLambda:                   %e",
                   header->omegalambda);
    io_logging_msg(log, INT32_C(5), "  Hubble parameter:              %e",
                   header->hubbleparameter);
    io_logging_msg(log, INT32_C(5), "  Flag Stellar Age:              %" PRIi32,
                   header->flagstellarage);
    io_logging_msg(log, INT32_C(5), "  Flag Metals:                   %" PRIi32,
                   header->flagmetals);
    io_logging_msg(log, INT32_C(5), "  No of particles in total (HW): %" PRIu32,
                   header->nallhighw[0]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
                   header->nallhighw[1]);
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
    ...
    io_logging_msg(log, INT32_C(5), "                                 %" PRIu32,
                   header->nallhighw[5]);
    io_logging_msg(log, INT32_C(5), "  Flag Entropy insted U:         %" PRIi32,
                   header->flagentropyu);
