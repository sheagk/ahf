...
int dustbin;
#define SKIP(fp)                                                               \
    { io_util_readint32(fp, &dustbin, f->swapped); }
...
extern io_art_header_t io_art_header_get(io_logging_t log, io_art_t f) {
...
#ifndef MULTIMASS
    io_logging_fatal(log, "you need to switch on -DMULTIMASS even if you only "
                          "have a one-species simulation!");
    return NULL;
#endif
    ...
    if (fPMcrd == NULL) {
        io_logging_fatal(
            log, "could not open PMcrd.DAT with ART header information");
        return NULL;
    }
    ...
    if (dummy->nspecies == 0) {
        ...
        dummy->N_pages =
            (dummy->N_particles - 1) / ((dummy->nrowc) * (dummy->nrowc)) + 1;
        dummy->N_in_last =
            dummy->N_particles -
            ((dummy->nrowc) * (dummy->nrowc)) * (dummy->N_pages - 1);
    }
    ...
    if (dummy->extras[100 - 1] > 0 && dummy->extras[59 - 1] > 0) {
        ...
        if ((fart = fopen("art.info", "r")) == NULL) {
            io_logging_fatal(log, "Could not open art.info containing the "
                                  "following information:");
            io_logging_fatal(
                log, "20.0                    ART_BOXSIZE           [Mpc/h]");
            io_logging_fatal(
                log, "2.2197e09               ART_MUNIT             [Msun/h]");
...
extern void io_art_header_log(io_logging_t log, io_art_header_t header) {
    io_logging_msg(log, INT32_C(5), "   aexpn:                        %e",
                   header->aexpn);
    io_logging_msg(log, INT32_C(5), "   aexp0:                        %e",
                   header->aexp0);
    io_logging_msg(log, INT32_C(5), "   astep:                        %e",
                   header->astep);
    io_logging_msg(log, INT32_C(5), "   istep:                        %" PRIi32,
                   header->istep);
    io_logging_msg(log, INT32_C(5), "   partw:                        %e",
                   header->partw);
    io_logging_msg(log, INT32_C(5), "   nrowc:                        %" PRIi32,
                   header->nrowc);
    io_logging_msg(log, INT32_C(5), "   ngridc:                       %" PRIi32,
                   header->ngridc);
    io_logging_msg(log, INT32_C(5), "   nspecies:                     %" PRIi32,
                   header->nspecies);
    io_logging_msg(log, INT32_C(5), "   Om0:                          %e",
                   header->Om0);
    io_logging_msg(log, INT32_C(5), "   Oml0:                         %e",
                   header->Oml0);
    io_logging_msg(log, INT32_C(5), "   extras[58] (pmass):           %e",
                   header->extras[58]);
    io_logging_msg(log, INT32_C(5), "   extras[99] (boxsize):         %e",
                   header->extras[99]);
