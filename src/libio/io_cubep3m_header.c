 ...
 */
inline static void local_null_header(io_cubep3m_header_t header);
 ...
 */
inline static int local_extract_file_number_from_name(const char *fname);
 ...
 * \return  Returns nothing.
 */
inline static void local_calculate_file_offset(int file_number, int nodes_dim,
                                               uint64_t ngrid, double *offset);
 ...
 */
inline static FILE *local_open_info_file(const char *info_file_name,
                                         io_logging_t log);
...
extern void io_cubep3m_header_read_extras(io_logging_t log,
    ...
    header->nptotal =
        (header->ngrid / 2) * (header->ngrid / 2) * (header->ngrid / 2);
...
extern void io_cubep3m_header_log(io_logging_t log,
                                  io_cubep3m_header_t header) {
    io_logging_msg(log, INT32_C(5), "   np_local:                     %" PRIi32,
                   header->np_local);
    io_logging_msg(log, INT32_C(5), "   a:                            %e",
                   header->a);
    io_logging_msg(log, INT32_C(5), "   t:                            %e",
                   header->t);
    io_logging_msg(log, INT32_C(5), "   tau:                          %e",
                   header->tau);
    io_logging_msg(log, INT32_C(5), "   nts:                          %" PRIi32,
                   header->nts);
    io_logging_msg(log, INT32_C(5), "   dt_f_acc:                     %e",
                   header->dt_f_acc);
    io_logging_msg(log, INT32_C(5), "   dt_pp_acc:                    %e",
                   header->dt_pp_acc);
    io_logging_msg(log, INT32_C(5), "   dt_c_acc:                     %e",
                   header->dt_c_acc);
    io_logging_msg(log, INT32_C(5), "   cur_checkpoint:               %" PRIi32,
                   header->cur_checkpoint);
    io_logging_msg(log, INT32_C(5), "   cur_projection:               %" PRIi32,
                   header->cur_projection);
    io_logging_msg(log, INT32_C(5), "   cur_halofind:                 %" PRIi32,
                   header->cur_halofind);
    io_logging_msg(log, INT32_C(5), "   mass_p:                       %e",
                   header->mass_p);

    io_logging_msg(log, INT32_C(5), "  Chunk information");
    io_logging_msg(log, INT32_C(5), "   tot_np_in_chunk:              %" PRIi64,
                   header->tot_np_in_chunk);
    io_logging_msg(log, INT32_C(5), "   np_in_chunk_file:             %" PRIi64,
                   header->np_in_chunk_file);
    io_logging_msg(log, INT32_C(5), "   num_chunk_files:              %i",
                   header->num_chunk_files);
    io_logging_msg(log, INT32_C(5), "   chunk_offset:                 %e %e %e",
                   header->chunk_offset[0], header->chunk_offset[1],
                   header->chunk_offset[2]);
    io_logging_msg(log, INT32_C(5), "   chunk_start_full_data:        %e %e %e",
                   header->chunk_start_full_data[0],
                   header->chunk_start_full_data[1],
                   header->chunk_start_full_data[2]);
    io_logging_msg(log, INT32_C(5), "   chunk_end_full_data:          %e %e %e",
                   header->chunk_end_full_data[0],
                   header->chunk_end_full_data[1],
                   header->chunk_end_full_data[2]);
    io_logging_msg(log, INT32_C(5), "   chunk_start_real_data:        %e %e %e",
                   header->chunk_start_real_data[0],
                   header->chunk_start_real_data[1],
                   header->chunk_start_real_data[2]);
    io_logging_msg(log, INT32_C(5), "   chunk_end_real_data:          %e %e %e",
    ...
    io_logging_msg(log, INT32_C(5), "   boxsize:                      %e",
                   header->boxsize);
    io_logging_msg(log, INT32_C(5), "   lunit:                        %e",
                   header->lunit);
    io_logging_msg(log, INT32_C(5), "   vunit:                        %e",
                   header->vunit);
    io_logging_msg(log, INT32_C(5), "   munit:                        %e",
                   header->munit);
    io_logging_msg(log, INT32_C(5), "   nptotal:                      %" PRIu64,
                   header->nptotal);
    io_logging_msg(log, INT32_C(5), "   ngrid:                        %" PRIu64,
                   header->ngrid);
    io_logging_msg(log, INT32_C(5), "   nodes_dim:                    %i",
                   header->mass_p);
    io_logging_msg(log, INT32_C(5), "  Derived information");
    io_logging_msg(log, INT32_C(5), "   file_number:                  %i",
                   header->file_number);
    io_logging_msg(log, INT32_C(5),
                   "   offset:                       %e  %e  %e",
...
inline static FILE *local_open_info_file(const char *info_file_name,
    ...
    if (f == NULL) {
        io_logging_fatal(log, "Could not open cubep3m.info containing the "
                              "following information:");
        io_logging_fatal(log, "0.3               CUBEP3M_OMEGA0");
        io_logging_fatal(log, "0.7               CUBEP3M_LAMBDA0");
        io_logging_fatal(log, "20.0              CUBEP3M_BOXSIZE [Mpc/h]");
        io_logging_fatal(log, "512               CUBEP3M_NGRID");
        io_logging_fatal(log, "10                CUBEP3M_NODES_DIM");
