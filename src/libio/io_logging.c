 ...
 */
#ifdef WITH_MPI
static int32_t local_countdigits(int64_t i);
#endif
 ...
 */
inline static int8_t local_logcheck(io_logging_t log, int32_t verbosity);
...
extern io_logging_t io_logging_start(char *stem, int32_t verbosity,
    ...
    if (stem == NULL) {
        fprintf(stderr,
                ...
                __func__, (unsigned int)__LINE__, IO_LOGGING_STEM_STANDARD);
        fname = (char *)malloc(sizeof(char) *
                               (1 + strlen(IO_LOGGING_STEM_STANDARD)));
    } else {
        fname = (char *)malloc(sizeof(char) * (1 + strlen(stem)));
    }
    if (fname == NULL) {
        fprintf(stderr, "%s, %u: Could not allocate memory for the stem.\n",
                __func__, (unsigned int)__LINE__);
        free(log);
        return NULL;
    }
    ...
    suffix =
        (char *)malloc(sizeof(char) * (digits + 2 + strlen(IO_LOGGING_SUFFIX)));
#else
    suffix = (char *)malloc(sizeof(char) * (1 + strlen(IO_LOGGING_SUFFIX)));
#endif /* WITH_MPI */
    if (suffix == NULL) {
        fprintf(stderr, "%s, %u: Could not allocate memory for suffix.\n",
                __func__, (unsigned int)__LINE__);
        free(fname);
        free(log);
        return NULL;
    ...
    /* Create the suffix */
#ifdef WITH_MPI
    sprintf(suffix, ".%0*" PRIi32 "%s", (int)digits, rank, IO_LOGGING_SUFFIX);
#else
    ...
    log->fname =
        (char *)malloc(sizeof(char) * (strlen(fname) + 1 + strlen(suffix)));
    if (log->fname == NULL) {
        fprintf(stderr, "%s, %u: Could not allocate memory for the filename.\n",
                __func__, (unsigned int)__LINE__);
        free(fname);
        free(suffix);
        free(log);
    ...
    if (log->logfile == NULL) {
        fprintf(stderr, "%s, %u: Could not open %s for reading/writing.\n",
...
extern void io_logging_identify(io_logging_t log) {
    ...
    if (log->logfile != NULL) {
        ...
        fprintf(log->logfile,
                ...
                IO_VERSION_MAJOR, IO_VERSION_MINOR, IO_VERSION_MICRO, size,
                rank);
    }
...
extern void io_logging_part(io_logging_t log, const char *frmt, ...) {
    ...
    fprintf(log->logfile, IO_LOGGING_PART_NUM_PRE "%c" IO_LOGGING_PART_NUM_POST,
            ((int)log->cnt_part));
...
extern void io_logging_section(io_logging_t log, const char *frmt, ...) {
    ...
    fprintf(log->logfile,
            ...
            "%c-%" PRIi32 IO_LOGGING_SECTION_NUM_POST,
...
extern void io_logging_subsection(io_logging_t log, const char *frmt, ...) {
    ...
    fprintf(log->logfile,
            ...
            "%c-%" PRIi32 ".%" PRIi32 IO_LOGGING_SUBSECTION_NUM_POST,
...
#ifdef PRINT_VERBOSE_LEVEL
    fprintf(log->critical,
            "%02" PRIi32 ": FATAL: Could not allocate memory for ", INT32_C(0));
#else
...
extern void io_logging_flush(io_logging_t log) { fflush(log->logfile); }
