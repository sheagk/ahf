 ...
 *
 *
 *   THIS FILE HAS RECENTLY (09/2013) BEEN DEBUGGED BY ROK ROSKAR...
 */
 ...
 */
inline static io_tipsy_t local_openopen(io_logging_t log, io_tipsy_t f,
                                        io_file_mode_t mode);
 ...
 * \return Nothing.
 */
inline static void local_openswapped(io_logging_t log, io_tipsy_t f,
 ...
 */
inline static io_tipsy_t local_opengetswap(io_logging_t log, io_tipsy_t f);
 ...
 * \return
 */
inline static int32_t local_write_common(io_logging_t log, io_tipsy_t f,
...
extern io_tipsy_t io_tipsy_open(io_logging_t log, char *fname,
    ...
    fprintf(stderr,
            "you have not defined -DGAS_PARTICLES for your TIPSY simulation\n");
...
extern uint64_t io_tipsy_readpart(io_logging_t log, io_tipsy_t f,
    ...
    /* And do the scaling */
#ifdef WITH_MPI
    io_tipsy_scale_global(log, f->mycomm, f->maxpos, f->minpos, &(f->mmass),
                          &(f->minweight), &(f->maxweight), &(f->sumweight));
#endif

    // f->mmass = 1.0;
    tmp = io_tipsy_scale_particles(
        log, f->maxpos, f->minpos, f->header->boxsize, f->header->time,
        f->posscale, f->header->vunit, f->header->eunit, f->mmass,
        particles_read, strg);
...
extern uint64_t io_tipsy_readpart_raw(io_logging_t log, io_tipsy_t f,
    ...
    \*******************************************************************/
    bytes_file =
        4; // TIPSY file stores every variable (int, float) as a 4 byte number
    ...
    io_logging_msg(log, INT32_C(1),
                   ...
                   (float)((double)blocksize / 1024. / 1024.));
    ...
    //===================================
    else if (pskip > f->header->nsph &&
             pskip <= f->header->nsph + f->header->ndark) {
        ...
        // skip over remaining dark particles consisting of TIPSY_DARKVAR
        // variables
    ...
    //===================================
             ...
             pskip <= f->header->nsph + f->header->ndark + f->header->nstar) {
        // skip over all gas particles consisting of TIPSY_GASVAR variables
        partsize = TIPSY_GASVAR * bytes_file;
#ifdef FSEEKO
        fseeko(f->file, partsize * f->header->nsph, SEEK_CUR);
        ...
        // skip over remaining star particles consisting of TIPSY_STARVAR
        // variables
        partsize = TIPSY_STARVAR * bytes_file;
#ifdef FSEEKO
        fseeko(f->file, partsize * (pskip - f->header->nsph + f->header->ndark),
               SEEK_CUR);
#else
        fseek(f->file,
              partsize * (pskip - (f->header->nsph + f->header->ndark)),
              SEEK_CUR);
#endif
    }
...
extern bool io_tipsy_get(io_logging_t log, io_tipsy_t f, io_file_get_t what,
    ...
    case IO_FILE_GET_PMASS:
        *((double *)res) = f->header->munit * f->mmass;
        break;
...
extern uint64_t io_tipsy_scale_particles(io_logging_t log, double maxpos[],
    ...
    B = 1.0; // the standard szenario is that we do not want to scale the
             // positions
...
\*=================================================================*/
    ...
    io_logging_msg(log, INT32_C(3), "             weights:    %g",
                   scale_weight);
    io_logging_msg(log, INT32_C(3), "             energies:   %g", scale_e);
        ...
        *((type *)strg.momz.val) *= (type)(scale_mom);                         \
        if (strg.u.val != NULL) {                                              \
            if (*((type *)strg.u.val) >= 0.)                                   \
                *((type *)strg.u.val) *= (type)(scale_e);                      \
        }                                                                      \
        if (strg.weight.val != NULL)                                           \
            *((type *)strg.weight.val) *= (type)(scale_weight);                \
        if (strg.rho.val != NULL)                                              \
            *((type *)strg.rho.val) *= (type)(scale_rho);                      \
        if (strg.eps.val != NULL)                                              \
            *((type *)strg.eps.val) *= (type)(scale_pos);                      \
        strg.posx.val = (void *)(((char *)strg.posx.val) + strg.posx.stride);  \
        strg.posy.val = (void *)(((char *)strg.posy.val) + strg.posy.stride);  \
        strg.posz.val = (void *)(((char *)strg.posz.val) + strg.posz.stride);  \
        strg.momx.val = (void *)(((char *)strg.momx.val) + strg.momx.stride);  \
