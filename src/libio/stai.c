...
extern stai_t stai_cloneWithDifferentBase(const stai_t stai, void *newBase) {
    ...
    return stai_new(newBase, stai->sizeOfElementInBytes, stai->strideInBytes);
}
...
extern void stai_setElement(const stai_t stai, uint64_t pos,
    ...
    memcpy((char *)(stai->base) + pos * stai->strideInBytes, element,
...
extern void stai_setElementsMulti(const stai_t stai, uint64_t pos,
    ...
    for (uint64_t i = 0; i < numElements; i++)
        stai_setElement(
            stai, pos + i,
            ((const char *)elements + stai->sizeOfElementInBytes * i));
}
...
extern void stai_getElement(const stai_t stai, uint64_t pos, void *element) {
    ...
    memcpy(element, (char *)(stai->base) + pos * stai->strideInBytes,
           stai->sizeOfElementInBytes);
}
