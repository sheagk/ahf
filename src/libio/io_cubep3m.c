 ...
 */
inline static io_cubep3m_t local_openopen(io_logging_t log, io_cubep3m_t f,
                                          io_file_mode_t mode);
 ...
 */
inline static void local_open_identify_file(io_logging_t log, io_cubep3m_t f,
 ...
 */
inline static io_file_swap_t local_getSwapStateFile(int32_t magicNumber);

inline static void local_print_offset_information(io_logging_t log,
                                                  io_cubep3m_t f);
...
extern uint64_t io_cubep3m_readpart(io_logging_t log, io_cubep3m_t f,
    ...
    /* And do the scaling */
    tmp = io_cubep3m_scale_particles(log, f->header->boxsize, f->header->a,
                                     f->header->lunit, f->header->vunit,
                                     particles_read, strg);
...
extern uint64_t io_cubep3m_readpart_raw(io_logging_t log, io_cubep3m_t f,
    ...
    if (strg.bytes_int == sizeof(uint32_t)) {
        io_logging_fatal(
            log,
            "You are planning to read 64bit particle ids, but the internal id "
            "storage is only designed for 32bit ids. Aborting in %s.",
            __func__);
    }
#endif
    ...
    fseek(f->file, (bytes_file * 6 + sizeof(uint64_t)) * pskip,
          SEEK_CUR); // pids are using uint64_t as storage
    ...
    for (i = pskip; i < pskip + pread; i++) {
        ...
        fposx = (float)fmod(fposx + f->header->offset[0] +
                                (double)(f->header->ngrid),
                            boxsizeFileInternal);
        fposy = (float)fmod(fposy + f->header->offset[1] +
                                (double)(f->header->ngrid),
                            boxsizeFileInternal);
        fposz = (float)fmod(fposz + f->header->offset[2] +
                                (double)(f->header->ngrid),
                            boxsizeFileInternal);
        ...
        if (strg.id.val != NULL) {
            ...
            *((uint64_t *)strg.id.val) = pid;
            ...
            io_logging_fatal(log,
                             "You are planning to read particle ids, but there "
                             "is no memory allocated for it. Aborting in %s.",
                             __func__);
        }
#else  /* CUBEP3M_WITH_PIDS */
...
extern void io_cubep3m_log(io_logging_t log, io_cubep3m_t f) {
    ...
    io_logging_msg(log, INT32_C(5), "  Swapping:             %" PRIi8,
                   f->swapped);
    io_logging_msg(log, INT32_C(5), "  isChunked:            %s",
...
#define SCALE_CALL(type)                                                       \
    {                                                                          \
        *((type *)strg.posx.val) =                                             \
            (type)fmod((*((type *)strg.posx.val)) * scale_pos, 1.0);           \
        *((type *)strg.posy.val) =                                             \
            (type)fmod((*((type *)strg.posy.val)) * scale_pos, 1.0);           \
        *((type *)strg.posz.val) =                                             \
            (type)fmod((*((type *)strg.posz.val)) * scale_pos, 1.0);           \
...
inline static void local_print_offset_information(io_logging_t log,
    ...
    io_logging_msg(log, INT32_C(1), "Offset information:", f->fname);
